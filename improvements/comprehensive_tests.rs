...
mod unit_tests {
    ...
    mod mariadb_tests {
        ...
        async fn test_transaction_rollback() {
            ...
            let mut tx = pool.begin().await.expect("Test assertion failed");
            ...
            assert!(result.is_err());
            tx.rollback().await.expect("Test assertion failed");
        }
    }
    ...
    mod tcp_protocol_tests {
        ...
        fn test_message_encoding_decoding() {
            ...
            let encoded = original.to_bytes().expect("Test assertion failed");
            let decoded = GameMessage::from_bytes(&encoded).expect("Test assertion failed");
    ...
    mod game_logic_tests {
        ...
        async fn test_player_movement_validation() {
            let state_manager = GameStateManager::new().await.expect("Test assertion failed");
            
            // 플레이어 추가
            let player_id = 1;
            state_manager.add_player(player_id, "TestPlayer").await.expect("Test assertion failed");
        ...
        async fn test_combat_system() {
            ...
            // 두 플레이어 추가
            state_manager.add_player(1, "Attacker").await.expect("Test assertion failed");
            state_manager.add_player(2, "Defender").await.expect("Test assertion failed");
            ...
            // 피해 확인
            let defender = state_manager.get_player(2).await.expect("Test assertion failed");
    ...
    mod performance_tests {
        ...
        async fn test_concurrent_connections() {
            ...
            let listener = TcpListener::bind("127.0.0.1:0").await.expect("Test assertion failed");
...
mod integration_tests {
    ...
    async fn test_full_game_flow() {
        ...
        // 3. 로그인
        client1.login("player1", "password1").await.expect("Test assertion failed");
        client2.login("player2", "password2").await.expect("Test assertion failed");
        
        // 4. 게임 플레이
        client1.move_to(Position::new(100.0, 100.0)).await.expect("Test assertion failed");
        client2.move_to(Position::new(110.0, 110.0)).await.expect("Test assertion failed");
        
        // 5. 전투
        client1.attack(client2.id()).await.expect("Test assertion failed");
        
        // 6. 채팅
        client1.send_chat("안녕하세요!").await.expect("Test assertion failed");
