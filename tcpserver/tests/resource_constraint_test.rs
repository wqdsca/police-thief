...
impl ResourceConstraintSimulator {
    ...
    pub async fn start_monitoring(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            while monitoring_active.load(Ordering::Relaxed) {
                ...
                // 주기적 상태 출력
                if stats.lock().await.len() % 15 == 0 {
                    // 30초마다 (2초 간격 * 15)
                    info!(
                        "📊 리소스 상태: 메모리 {}MB ({:.1}%), CPU {:.1}%, 가동시간 {}초",
    ...
    fn collect_resource_status(
        ...
        let memory_usage_percent =
            (current_memory_mb as f32 / constraints.max_memory_mb as f32) * 100.0;
        let cpu_usage_percent = current_cpu_percent / constraints.max_cpu_percent * 100.0;

        // 경고 및 위험 상태 판단
        let is_memory_warning =
            memory_usage_percent > constraints.warning_thresholds.memory_warning_percent;
    ...
    pub async fn get_stats(&self) -> ResourceStats {
        ...
        let max_cpu_percent = stats_vec
            .iter()
            .map(|s| s.current_cpu_percent)
            .fold(0.0f32, |a, b| a.max(b));

        let warning_count = stats_vec
            .iter()
            .filter(|s| s.is_memory_warning || s.is_cpu_warning)
            .count() as u32;

        let critical_count = stats_vec.iter().filter(|s| s.is_critical).count() as u32;
    ...
    pub async fn evaluate_compliance(&self) -> ComplianceReport {
        ...
        let cpu_efficiency =
            (stats.avg_cpu_percent / self.constraints.max_cpu_percent as f64) * 100.0;
    ...
    pub async fn print_detailed_report(&self) {
        ...
        println!(
            "   평균 메모리: {:.1} MB ({:.1}%)",
            report.stats.avg_memory_mb, report.memory_efficiency
        );
...
async fn test_resource_constraint_compliance() -> Result<()> {
    ...
    let report =
        run_resource_constraint_test("기본 리소스 제약 테스트", constraints, || async {
            // 가벼운 워크로드 시뮬레이션
...
async fn test_heavy_workload_constraint() -> Result<()> {
    ...
    let report = run_resource_constraint_test(
        ...
        || async {
            ...
            cpu_result.expect("Test assertion failed");
            memory_result.expect("Test assertion failed");
