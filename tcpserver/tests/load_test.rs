...
pub enum LoadTestMessage {
    ...
    RoomMessage { room_id: u32, user_id: u32, message: String, timestamp: i64 },
    HeartBeat,
    ...
    pub async fn start_monitoring(&self, duration: Duration) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            while start.elapsed() < duration {
                ...
                debug!("리소스 모니터링: CPU {:.1}%, 메모리 {} KB", cpu_usage, memory_usage);
            }
        })
    }
...
impl VirtualUser {
    pub fn new(user_id: u32, room_id: u32, stats: Arc<LoadTestStats>, connected: Arc<AtomicUsize>) -> Self {
        Self {
            user_id,
            room_id,
            stream: None,
        ...
        while start.elapsed() < duration {
            ...
            if let Some(ref mut stream) = self.stream {
                ...
                let room_msg = LoadTestMessage::RoomMessage {
                    room_id: self.room_id,
                    user_id: self.user_id,
                    message: format!("Hello from user {} - message {}", self.user_id, message_counter),
                ...
                match room_msg.write_to_stream(stream).await {
                    Ok(_) => {
                        ...
                        if let Ok(response) = timeout(Duration::from_millis(500), 
                            LoadTestMessage::read_from_stream(stream)).await {
                            ...
                            match response {
                                Ok(_) => {
                                    ...
                                    self.stats.room_message_responses.fetch_add(1, Ordering::Relaxed);
                                }
                                Err(_) => {
    ...
    pub async fn send_heartbeat(&mut self) -> Result<()> {
        if let Some(ref mut stream) = self.stream {
            ...
            // 하트비트 응답 대기
            match timeout(Duration::from_secs(5), LoadTestMessage::read_from_stream(stream)).await {
                Ok(Ok(LoadTestMessage::HeartBeatResponse { .. })) => {
                    self.stats.heartbeat_responses.fetch_add(1, Ordering::Relaxed);
                    Ok(())
                }
...
impl LoadTester {
    ...
    pub async fn run_load_test(&self) -> Result<()> {
        ...
        // 리소스 모니터링 시작
        let monitor_handle = self.resource_monitor.start_monitoring(self.test_duration + Duration::from_secs(10)).await;
    ...
    async fn print_final_results(&self) {
        ...
        // 리소스 사용량 요약
        let (avg_cpu, max_cpu, avg_memory_kb, max_memory_kb) = self.resource_monitor.get_summary().await;
        ...
        println!("💾 메모리 사용률: {:.1}% / 100% (512MB)", memory_usage_percent);
...
async fn test_small_scale_load() -> Result<()> {
    ...
    // 서버 연결 확인
    if TcpStream::connect(&load_tester.server_address).await.is_err() {
...
async fn test_connection_stress() -> Result<()> {
    ...
    info!("연결 테스트 결과: 성공 {}, 실패 {}", final_connected, connection_failures);
