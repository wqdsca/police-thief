...
pub enum OptimizedGameMessage {
    ...
    ChatResponse { success: bool, error: Option<String> } = 0x11,
    
    // 방 관리
    RoomJoin { user_id: u32, room_id: u32, nickname: String } = 0x20,
    RoomLeave { user_id: u32, room_id: u32 } = 0x21,
    RoomJoinSuccess { room_id: u32, user_count: u32 } = 0x22,
    RoomLeaveSuccess { room_id: u32, user_count: u32 } = 0x23,
    
    // 사용자 알림
    UserJoinedRoom { room_id: u32, user_id: u32, nickname: String, user_count: u32 } = 0x30,
    UserLeftRoom { room_id: u32, user_id: u32, nickname: String, user_count: u32 } = 0x31,
    
    // 시스템 메시지
    SystemMessage { message: String } = 0x40,
...
impl OptimizedGameMessage {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        ...
        match self {
            ...
            OptimizedGameMessage::ChatMessage { user_id, room_id, message } => {
                buffer.push(0x10);
                buffer.extend_from_slice(&user_id.to_le_bytes());
            ...
            OptimizedGameMessage::RoomJoin { user_id, room_id, nickname } => {
                buffer.push(0x20);
                buffer.extend_from_slice(&user_id.to_le_bytes());
                buffer.extend_from_slice(&room_id.to_le_bytes());
                Self::write_string(&mut buffer, nickname)?;
            ...
            OptimizedGameMessage::RoomJoinSuccess { room_id, user_count } => {
                buffer.push(0x22);
                buffer.extend_from_slice(&room_id.to_le_bytes());
                buffer.extend_from_slice(&user_count.to_le_bytes());
            },
            
            OptimizedGameMessage::RoomLeaveSuccess { room_id, user_count } => {
                buffer.push(0x23);
                buffer.extend_from_slice(&room_id.to_le_bytes());
                buffer.extend_from_slice(&user_count.to_le_bytes());
            },
            
            OptimizedGameMessage::UserJoinedRoom { room_id, user_id, nickname, user_count } => {
                buffer.push(0x30);
                buffer.extend_from_slice(&room_id.to_le_bytes());
                buffer.extend_from_slice(&user_id.to_le_bytes());
                Self::write_string(&mut buffer, nickname)?;
            ...
            OptimizedGameMessage::UserLeftRoom { room_id, user_id, nickname, user_count } => {
                buffer.push(0x31);
                buffer.extend_from_slice(&room_id.to_le_bytes());
                buffer.extend_from_slice(&user_id.to_le_bytes());
                Self::write_string(&mut buffer, nickname)?;
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        match msg_type {
            ...
            0x10 => {
                ...
                Ok(OptimizedGameMessage::ChatMessage { user_id, room_id, message })
            },
            ...
            0x20 => {
                ...
                let room_id = Self::read_u32(&mut cursor)?;
                let nickname = Self::read_string(&mut cursor)?;
                Ok(OptimizedGameMessage::RoomJoin { user_id, room_id, nickname })
            },
            ...
            0x22 => {
                ...
                Ok(OptimizedGameMessage::RoomJoinSuccess { room_id, user_count })
            },
            
            0x23 => {
                let room_id = Self::read_u32(&mut cursor)?;
                let user_count = Self::read_u32(&mut cursor)?;
                Ok(OptimizedGameMessage::RoomLeaveSuccess { room_id, user_count })
            },
            ...
            0x30 => {
                ...
                let nickname = Self::read_string(&mut cursor)?;
                let user_count = Self::read_u32(&mut cursor)?;
                Ok(OptimizedGameMessage::UserJoinedRoom { room_id, user_id, nickname, user_count })
            },
            ...
            0x31 => {
                ...
                let nickname = Self::read_string(&mut cursor)?;
                let user_count = Self::read_u32(&mut cursor)?;
                Ok(OptimizedGameMessage::UserLeftRoom { room_id, user_id, nickname, user_count })
            },
        ...
        W: AsyncWrite + Unpin,
    {
        ...
        debug!("메시지 전송: 타입={:?}, 크기={}바이트", self.message_type(), data.len());
        Ok(())
    }
        ...
        R: AsyncRead + Unpin,
    {
        ...
        debug!("메시지 수신: 타입={:?}, 크기={}바이트", message.message_type(), len);
...
impl OptimizedGameMessage {
    ...
    pub fn from_game_message(msg: &crate::protocol::GameMessage) -> Self {
        match msg {
            ...
            crate::protocol::GameMessage::ChatMessage { user_id, room_id, message } => {
            ...
            crate::protocol::GameMessage::RoomJoin { user_id, room_id, nickname } => {
            ...
            crate::protocol::GameMessage::RoomJoinSuccess { room_id, user_count } => {
                OptimizedGameMessage::RoomJoinSuccess { 
                    room_id: *room_id, 
                    user_count: *user_count 
                }
            },
            crate::protocol::GameMessage::RoomLeaveSuccess { room_id, user_count } => {
                OptimizedGameMessage::RoomLeaveSuccess { 
                    room_id: *room_id, 
                    user_count: *user_count 
                }
            },
            crate::protocol::GameMessage::UserJoinedRoom { room_id, user_id, nickname, user_count } => {
            ...
            crate::protocol::GameMessage::UserLeftRoom { room_id, user_id, nickname, user_count } => {
            ...
            crate::protocol::GameMessage::Error { code, message } => {
                OptimizedGameMessage::Error { 
                    code: *code, 
                    message: message.clone() 
                }
            },
    ...
    pub fn to_game_message(&self) -> crate::protocol::GameMessage {
        match self {
            ...
            OptimizedGameMessage::Error { code, message } => {
                crate::protocol::GameMessage::Error { 
                    code: *code, 
                    message: message.clone() 
                }
            },
...
impl ProtocolBenchmark {
    ...
    pub fn compare_serialization_performance(iterations: usize) -> (f64, f64, f64) {
        ...
        let test_message = OptimizedGameMessage::ChatMessage {
            user_id: 12345,
            room_id: 67890,
            message: "안녕하세요! 이것은 성능 테스트 메시지입니다. 한국어도 잘 동작하나요?".to_string(),
        };
        ...
        for _ in 0..iterations {
            let _json = serde_json::to_string(&json_message).unwrap();
        }
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            let _binary = test_message.to_bytes().unwrap();
        }
        let binary_serialize_time = start.elapsed().as_millis() as f64;
        
        // 크기 비교
        let json_size = serde_json::to_string(&json_message).unwrap().len();
        let binary_size = test_message.to_bytes().unwrap().len();
        let size_ratio = json_size as f64 / binary_size as f64;
        let speed_ratio = json_serialize_time / binary_serialize_time;
        
        (size_ratio, speed_ratio, binary_serialize_time / json_serialize_time)
    }
}
...
mod tests {
    ...
    fn test_heartbeat_serialization() {
        ...
        let decoded = OptimizedGameMessage::from_bytes(&bytes).unwrap();
    ...
    fn test_performance_comparison() {
        let (size_ratio, speed_ratio, efficiency) = ProtocolBenchmark::compare_serialization_performance(1000);
    ...
    async fn test_async_stream_operations() {
        ...
        // 전송
        msg.write_to_async_stream(&mut client).await.unwrap();
        
        // 수신
        let received = OptimizedGameMessage::read_from_async_stream(&mut server).await.unwrap();
