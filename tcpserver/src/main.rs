...
use config::{validate_config, TcpServerConfig};
use handler::{ConnectionHandler, FriendHandler, RoomHandler, ServerMessageHandler};
use service::{ConnectionService, HeartbeatService, MessageService};
use shared::service_factory::DefaultServiceFactory;
...
impl SimpleTcpServer {
    /// 새로운 간단한 TCP 서버 생성 (의존성 주입 사용)
    pub async fn new() -> Self {
        info!("TCP 서버 초기화 - 의존성 주입 사용");

        // Service Factory를 통한 의존성 생성
