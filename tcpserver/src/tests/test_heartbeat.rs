...
async fn test_heartbeat_start_stop() {
    ...
    // 시작 테스트
    heartbeat_service.start().await.expect("Test assertion failed");
    assert!(heartbeat_service.is_running().await, "시작 후에는 실행 중이어야 함");
    
    // 중지 테스트
    heartbeat_service.stop().await.expect("Test assertion failed");
...
async fn test_manual_cleanup() {
    ...
    // 연결이 없는 상태에서 수동 정리
    let cleanup_count = heartbeat_service.cleanup_now().await.expect("Test assertion failed");
