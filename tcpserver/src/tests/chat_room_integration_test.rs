...
async fn test_room_join_leave() {
    ...
    assert!(result.is_ok());
    let user_count = result.expect("Test assertion failed");
    ...
    let room_deleted = leave_result.expect("Test assertion failed");
...
async fn test_multiple_users_room() {
    ...
    assert_eq!(result1.expect("Test assertion failed"), 1);
    ...
    assert_eq!(result2.expect("Test assertion failed"), 2);
    ...
    assert_eq!(result3.expect("Test assertion failed"), 3);
...
async fn test_chat_messaging() {
    ...
    env.add_test_user(300, 1, "채터1")
        .await
        .expect("Test assertion failed");
    env.add_test_user(300, 2, "채터2")
        .await
        .expect("Test assertion failed");
    env.add_test_user(300, 3, "채터3")
        .await
        .expect("Test assertion failed");
    ...
    let chat_result = env
        .chat_handler
        .handle_chat_message(1, 300, "안녕하세요!".to_string())
        .await;

    assert!(chat_result.is_ok());
    let sent_count = chat_result.expect("Test assertion failed");
    assert_eq!(sent_count, 3); // 방에 있는 모든 사용자(3명)에게 전송

    // 빈 메시지 테스트
    let empty_chat_result = env
        .chat_handler
        .handle_chat_message(1, 300, "".to_string())
        .await;
    assert!(empty_chat_result.is_err());

    // 존재하지 않는 방에 메시지 전송 테스트
    let invalid_room_result = env
        .chat_handler
        .handle_chat_message(1, 999, "존재하지 않는 방".to_string())
        .await;
...
async fn test_user_disconnect() {
    ...
    // 사용자들을 여러 방에 추가
    env.add_test_user(400, 1, "연결자1")
        .await
        .expect("Test assertion failed");
    env.add_test_user(400, 2, "연결자2")
        .await
        .expect("Test assertion failed");
    env.add_test_user(401, 3, "연결자3")
        .await
        .expect("Test assertion failed");
    ...
    let cleaned_rooms = disconnect_result.expect("Test assertion failed");
...
async fn test_concurrent_room_operations() {
    ...
    for handle in handles {
        let result = handle.await.expect("Test assertion failed");
        assert!(result.is_ok());
    }
    ...
    for user_id in 1..=10 {
        ...
        let handle =
            tokio::spawn(
                async move { env_clone.chat_handler.handle_room_leave(user_id, 700).await },
            );
    ...
    for handle in leave_handles {
        let result = handle.await.expect("Test assertion failed");
        assert!(result.is_ok());
        if result.expect("Test assertion failed") {
            room_deletions += 1;
        }
    }
...
async fn test_error_handling() {
    ...
    let chat_result = env
        .chat_handler
        .handle_chat_message(999, 999, "존재하지 않는 방".to_string())
        .await;
