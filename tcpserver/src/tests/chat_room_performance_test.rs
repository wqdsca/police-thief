...
impl PerformanceMetrics {
    ...
    fn get_summary(&self) -> PerformanceSummary {
        ...
        let avg_response_time_ms = if samples > 0 {
            (self.total_response_time_ns.load(Ordering::Relaxed) as f64 / samples as f64)
                / 1_000_000.0
        } else {
            0.0
        };
        ...
        PerformanceSummary {
            ...
            success_rate: if self.total_messages.load(Ordering::Relaxed) > 0 {
                (self.successful_messages.load(Ordering::Relaxed) as f64
                    / self.total_messages.load(Ordering::Relaxed) as f64)
                    * 100.0
            } else {
                0.0
            },
...
impl PerformanceTestEnv {
    ...
    async fn create_mock_writer(
    ) -> Arc<Mutex<tokio::io::BufWriter<tokio::net::tcp::OwnedWriteHalf>>> {
        use tokio::net::{TcpListener, TcpStream};

        let listener = TcpListener::bind("127.0.0.1:0")
            .await
            .expect("Test assertion failed");
        let addr = listener.local_addr().expect("Test assertion failed");
        ...
        let (server_stream, _) = listener.accept().await.expect("Test assertion failed");
        let _client_stream = client_task.await.expect("Test assertion failed");
...
async fn test_low_spec_performance_20_rooms_200_users_3min() {
    ...
    info!(
        "🚀 저사양 성능 테스트 시작 - 방 {}개, 사용자 {}명, {}초간 지속",
        TEST_ROOMS, TEST_TOTAL_USERS, TEST_DURATION_SECS
    );
    ...
    for handle in join_handles {
        handle.await.expect("Test assertion failed");
    }
    ...
    info!(
        "📊 방에 있는 총 사용자 수: {}, 예상: {}",
        total_users_in_rooms, TEST_TOTAL_USERS
    );
    ...
    for user_id in 1..=TEST_TOTAL_USERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            while Instant::now() < test_end_time {
                ...
                let result = env_clone
                    .chat_handler
                    .handle_chat_message(user_id, room_id, message_content)
                    .await;
    ...
    for handle in chat_handles {
        let messages = handle.await.expect("Test assertion failed");
        total_messages_sent += messages;
    }

    let chat_duration = chat_start.elapsed();
    info!(
        "✅ Phase 2 완료 - 채팅 시간: {:?}, 전송된 메시지: {}",
        chat_duration, total_messages_sent
    );
    ...
    info!(
        "🧹 빈 방 정리 완료: {}개, 시간: {:?}",
        cleaned_rooms, cleanup_duration
    );
    ...
    info!(
        "📊 예상 총 메시지: {}, 실제 전송: {}",
        expected_total_messages, total_messages_sent
    );

    // 저사양 환경 성능 기준 (더 관대한 기준)
    assert!(
        summary.successful_messages >= expected_total_messages * 90 / 100,
        "메시지 성공률이 90% 미만: {}/{}",
    ...
    assert!(
        actual_test_duration_secs >= TEST_DURATION_SECS - 5,
        "테스트가 예상 시간보다 너무 일찍 끝남: {}초 (예상: {}초)",
        actual_test_duration_secs,
        TEST_DURATION_SECS
    );
    ...
    info!(
        "📈 처리량: {:.2} 메시지/초",
        summary.successful_messages as f64 / actual_test_duration_secs as f64
    );
    info!(
        "💾 메모리 효율성: 방 {}개, 사용자 {}명 동시 처리",
        TEST_ROOMS, TEST_TOTAL_USERS
    );
}
...
async fn test_basic_performance_50_rooms_300_users() {
    ...
    info!(
        "📊 방에 있는 총 사용자 수: {}, 예상: {}",
        total_users_in_rooms, TOTAL_USERS
    );
    ...
    assert!(
        summary.successful_messages >= expected_total_messages * 95 / 100,
        "메시지 성공률이 95% 미만: {}/{}",
        summary.successful_messages,
        expected_total_messages
    );
    ...
    assert!(
        summary.avg_response_time_ms < 100.0,
        "평균 응답 시간이 100ms 초과: {:.2}ms",
        summary.avg_response_time_ms
    );
...
async fn test_concurrent_stress() {
    ...
    for user_id in 1..=TOTAL_USERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            for i in 1..=5 {
                let _ = env_clone
                    .chat_handler
                    .handle_chat_message(user_id, room_id, format!("스트레스메시지{}", i))
                    .await;
            }
            ...
            let _ = env_clone
                .chat_handler
                .handle_room_leave(user_id, room_id)
                .await;
        });
    ...
    assert!(
        final_rooms.is_empty() || final_rooms.iter().all(|(_, count)| *count == 0),
        "모든 방이 비어있어야 함"
    );
