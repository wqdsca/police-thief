...
impl NetworkUtils {
    ...
    pub async fn test_connection(addr: &str, timeout_secs: u64) -> Result<bool> {
        ...
        match timeout(Duration::from_secs(timeout_secs), TcpStream::connect(socket_addr)).await {
            Ok(Ok(_)) => {
...
mod tests {
    ...
    #[test]
    fn test_ip_parsing() {
        let ipv4 = NetworkUtils::parse_ip("192.168.1.1").unwrap();
    ...
    fn test_localhost_detection() {
        ...
        let external = "8.8.8.8".parse::<IpAddr>().unwrap();
    ...
    fn test_bind_address_normalization() {
        let port_only = NetworkUtils::normalize_bind_address("8080").unwrap();
        let full_addr = NetworkUtils::normalize_bind_address("127.0.0.1:8080").unwrap();
