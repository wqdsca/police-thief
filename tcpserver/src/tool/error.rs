...
impl fmt::Display for TcpServerError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TcpServerError::Connection { client_id, addr, message } => {
                write!(f, "연결 에러")?;
            ...
            TcpServerError::Protocol { message_type, raw_data, message } => {
                write!(f, "프로토콜 에러")?;
            ...
            TcpServerError::Heartbeat { client_id, operation, message } => {
                write!(f, "하트비트 에러")?;
                if let Some(id) = client_id {
                    write!(f, " [클라이언트 {}]", id)?;
                }
            ...
            TcpServerError::Service { service_name, operation, message } => {
                write!(f, "서비스 에러 [{}] [작업: {}]: {}", service_name, operation, message)
            }
            TcpServerError::Network { addr, operation, message } => {
                write!(f, "네트워크 에러")?;
                if let Some(address) = addr {
                    write!(f, " [{}]", address)?;
                }
...
macro_rules! handle_tcp_error {
    ($error:expr, $severity:expr, $component:expr, $operation:expr) => {
        $crate::tool::error::ErrorHandler::handle_error(
            $error,
            $severity,
            $component,
            $operation,
