...
impl AdaptiveBufferManager {
    ...
    async fn adjust_buffer_size(&self, sizes: &[usize]) {
        ...
        let new_size = (avg + 2 * std_dev).max(self.min_size).min(self.max_size);
...
impl AsyncIoOptimizer {
        ...
        W: AsyncWriteExt + Unpin + Send,
    {
        ...
        debug!(
            "파이프라인 전송 완료: {}바이트, 지연시간: {:?}",
            total_copied, total_latency
        );
    ...
    pub fn get_stats(&self) -> IoStats {
        IoStats {
            ...
            vectored_operations: AtomicU64::new(
                self.stats.vectored_operations.load(Ordering::Relaxed),
            ),
            coalesced_operations: AtomicU64::new(
                self.stats.coalesced_operations.load(Ordering::Relaxed),
            ),
            ...
            avg_read_latency_us: AtomicU64::new(
                self.stats.avg_read_latency_us.load(Ordering::Relaxed),
            ),
            avg_write_latency_us: AtomicU64::new(
                self.stats.avg_write_latency_us.load(Ordering::Relaxed),
            ),
        }
    }
    ...
    pub fn generate_performance_report(&self) -> AsyncIoPerformanceReport {
        ...
        let total_ops =
            stats.total_reads.load(Ordering::Relaxed) + stats.total_writes.load(Ordering::Relaxed);
    ...
    fn calculate_throughput(&self, stats: &IoStats) -> f64 {
        let total_bytes =
            stats.bytes_read.load(Ordering::Relaxed) + stats.bytes_written.load(Ordering::Relaxed);
        let total_time_us = (stats.avg_read_latency_us.load(Ordering::Relaxed)
            + stats.avg_write_latency_us.load(Ordering::Relaxed))
            / 2;
...
mod tests {
    ...
    async fn test_zero_copy_buffer_pool() {
        ...
        let buffer1 = pool.acquire().await.ok();
        assert_eq!(buffer1.capacity(), 4096);

        let buffer2 = pool.acquire().await.ok();
        assert_eq!(buffer2.capacity(), 4096);

        pool.release(buffer1).await;

        let buffer3 = pool.acquire().await.ok();
