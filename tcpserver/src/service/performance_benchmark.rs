...
use anyhow::Result;
// Note: rayon is available for future parallel processing needs
...
use crate::service::{
    AsyncIoOptimizer, AsyncIoOptimizerConfig, MessageCompressionConfig, MessageCompressionService,
    PerformanceMonitor, PerformanceMonitorConfig, SimdOptimizer, SimdOptimizerConfig,
};
...
impl PerformanceBenchmark {
    ...
    pub async fn benchmark_dashmap_optimizer(&self) -> Result<BenchmarkResult> {
        ...
        let min_latency = *latencies.iter().min().unwrap_or(&Duration::from_secs(0));
        let max_latency = *latencies.iter().max().unwrap_or(&Duration::from_secs(0));
        ...
        self.results
            .lock()
            .await
            .insert("dashmap_optimizer".to_string(), result.clone());

        info!(
            "✅ DashMap 최적화 벤치마크 완료: {:.1} ops/sec",
    ...
    pub async fn benchmark_async_io_optimizer(&self) -> Result<BenchmarkResult> {
        ...
        self.results
            .lock()
            .await
            .insert("async_io_optimizer".to_string(), result.clone());

        info!(
            "✅ 비동기 I/O 최적화 벤치마크 완료: {:.1} ops/sec",
            result.throughput_ops_per_sec
        );
        Ok(result)
    }
    ...
    pub async fn benchmark_simd_optimizer(&self) -> Result<BenchmarkResult> {
        ...
        self.results
            .lock()
            .await
            .insert("simd_optimizer".to_string(), result.clone());

        info!(
            "✅ SIMD 최적화 벤치마크 완료: {:.1} ops/sec",
            result.throughput_ops_per_sec
        );
        Ok(result)
    }
    ...
    pub async fn benchmark_message_compression(&self) -> Result<BenchmarkResult> {
        ...
        // 테스트 메시지 생성
        let test_message =
            "Hello, World! This is a test message for compression benchmarking. ".repeat(50);
        ...
        self.results
            .lock()
            .await
            .insert("message_compression".to_string(), result.clone());

        info!(
            "✅ 메시지 압축 벤치마크 완료: {:.1} ops/sec",
            result.throughput_ops_per_sec
        );
        Ok(result)
    }
    ...
    pub async fn benchmark_performance_monitor(&self) -> Result<BenchmarkResult> {
        ...
        for i in 0..self.config.iterations {
            ...
            // 메트릭 기록
            monitor
                .record_latency("test_latency", Duration::from_millis(10))
                .await;
        ...
        self.results
            .lock()
            .await
            .insert("performance_monitor".to_string(), result.clone());

        info!(
            "✅ 성능 모니터링 벤치마크 완료: {:.1} ops/sec",
            result.throughput_ops_per_sec
        );
        Ok(result)
    }
        ...
        for (name, result, duration) in results {
            match result {
                Ok(benchmark_result) => {
                    info!(
                        "✅ {} 벤치마크: 점수 {:.1}/100, 소요시간 {:?}",
                        benchmark_result.test_name,
                        benchmark_result.performance_score(),
                        duration
                    );
    ...
    pub async fn run_parallel_benchmarks(&self) -> Result<HashMap<String, BenchmarkResult>> {
        ...
        let handles = vec![
            tokio::spawn({
                ...
                async move { ("dashmap", benchmark.benchmark_dashmap_optimizer().await) }
            }),
            tokio::spawn({
                let benchmark = self.clone();
                async move { ("simd", benchmark.benchmark_simd_optimizer().await) }
            }),
            tokio::spawn({
                let benchmark = self.clone();
                async move {
                    (
                        "compression",
                        benchmark.benchmark_message_compression().await,
                    )
                }
            }),
        ];
...
mod tests {
    ...
    async fn test_dashmap_benchmark() {
        ...
        let result = benchmark.benchmark_dashmap_optimizer().await.ok();
    ...
    async fn test_full_benchmark_suite() {
        ...
        let benchmark = PerformanceBenchmark::new(config);
        let results = benchmark.run_all_benchmarks().await.ok();
