...
impl PerformanceMonitor {
        ...
        *self.monitoring_handle.lock().await = Some(monitoring_handle);
        ...
        let reporting_handle = tokio::spawn(async move {
            ...
            loop {
                ...
                // 성능 보고서 생성 및 로깅
                let cpu = collector
                    .get_latest_metric(MetricType::CpuUsage)
                    .await
                    .unwrap_or(0.0);
                let memory = collector
                    .get_latest_metric(MetricType::MemoryUsage)
                    .await
                    .unwrap_or(0.0);

                info!("📊 성능 보고서: CPU={:.1}%, 메모리={:.1}%", cpu, memory);
            }
        });
    ...
    pub async fn set_connection_count(&self, count: usize) {
        self.collector
            .set_gauge("active_connections", count as u64)
            .await;
        self.collector
            .record_metric(
    ...
    pub async fn generate_report(&self) -> PerformanceReport {
        ...
        let latency_p50 = histograms
            .get("message_latency")
            .map(|h| h.percentile(50.0))
            .unwrap_or(0.0);
        let latency_p95 = histograms
            .get("message_latency")
            .map(|h| h.percentile(95.0))
            .unwrap_or(0.0);
        let latency_p99 = histograms
            .get("message_latency")
