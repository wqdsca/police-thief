...
impl MemoryPoolStats {
    ...
    pub fn record_reuse(&self, memory_saved: u64) {
        ...
        self.memory_saved_bytes.fetch_add(memory_saved, Ordering::Relaxed);
...
impl BufferPool {
    ...
    pub fn rent(&self) -> PooledBuffer {
        ...
        if let Some(mut buffer) = self.pool.pop() {
            ...
            debug!("버퍼 재사용: 용량={}바이트, 재사용횟수={}", 
                   buffer.capacity(), buffer.total_reuses);
            buffer
        } else {
            // 새 버퍼 할당
    ...
    pub async fn cleanup_idle_buffers(&self) -> usize {
        ...
        while let Some(buffer) = self.pool.pop() {
            if buffer.is_idle(self.config.idle_threshold_secs) {
                cleaned += 1;
                debug!("유휴 버퍼 정리: 용량={}바이트, 유휴시간={}초", 
                       buffer.capacity(), buffer.last_used.elapsed().as_secs());
            } else {
        ...
        let new_size = self.current_size.load(Ordering::Relaxed).saturating_sub(cleaned);
        self.current_size.store(new_size, Ordering::Relaxed);
        self.stats.update_pool_size(new_size);
        
        if cleaned > 0 {
            info!("유휴 버퍼 {}개 정리 완료, 현재 풀 크기: {}", cleaned, new_size);
        }
...
pub struct ConnectionPool {
    pool: Arc<Mutex<VecDeque<Arc<Mutex<BufWriter<OwnedWriteHalf>>>>>>,
    max_size: usize,
    stats: Arc<MemoryPoolStats>,
}
...
impl ConnectionPool {
    ...
    pub async fn rent(&self) -> Option<Arc<Mutex<BufWriter<OwnedWriteHalf>>>> {
        ...
        if let Some(connection) = pool.pop_front() {
            self.stats.record_reuse(std::mem::size_of::<BufWriter<OwnedWriteHalf>>() as u64);
            debug!("연결 재사용");
            Some(connection)
        } else {
            self.stats.record_allocation();
    ...
    pub async fn return_connection(&self, connection: Arc<Mutex<BufWriter<OwnedWriteHalf>>>) {
        ...
        if pool.len() < self.max_size {
            ...
            self.stats.update_pool_size(pool.len());
            debug!("연결 풀에 반환, 현재 풀 크기: {}", pool.len());
        } else {
...
impl MemoryPoolManager {
    /// 새로운 메모리 풀 관리자 생성
    pub fn new(buffer_config: Option<BufferPoolConfig>, connection_pool_size: Option<usize>) -> Self {
