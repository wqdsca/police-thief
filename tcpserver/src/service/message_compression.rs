...
impl MessageBatch {
    ...
    pub fn deserialize(data: &[u8]) -> Result<Vec<Bytes>> {
        ...
        let mut cursor = 0;
        let message_count = u32::from_le_bytes([data[0], data[1], data[2], data[3]]) as usize;
        cursor += 4;
...
impl Default for AdaptiveCompressionManager {
    fn default() -> Self {
        Self::new()
    }
}
...
impl AdaptiveCompressionManager {
    ...
    pub async fn record_compression(
        ...
        // 알고리즘 통계 업데이트
        let entry = stats.iter_mut().find(|(alg, _, _)| *alg == algorithm);
...
impl MessageCompressionService {
    ...
    pub fn new(config: MessageCompressionConfig) -> Self {
        let cache = lru::LruCache::new(
            std::num::NonZeroUsize::new(config.cache_size).expect("Cache size must be non-zero"),
        );
    ...
    pub async fn compress(&self, data: &[u8]) -> Result<(Bytes, CompressionAlgorithm)> {
        ...
        if self.config.enable_adaptive_compression {
            self.adaptive_manager
                .record_compression(algorithm, data.len(), compressed.len(), duration)
                .await;
        }
    ...
    fn compress_zlib(&self, data: &[u8]) -> Result<Bytes> {
        let mut encoder =
            ZlibEncoder::new(Vec::new(), Compression::new(self.config.compression_level));
        encoder.write_all(data)?;
        let compressed = encoder.finish()?;
        Ok(Bytes::from(compressed))
    }
    ...
    pub async fn add_to_batch(&self, message: Bytes) -> Option<Bytes> {
        ...
        if batch.is_ready(&self.config) {
            ...
            self.stats
                .batched_messages
                .fetch_add(ready_batch.messages.len() as u64, Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_compression() {
        ...
        let (compressed, algorithm) = service.compress(data).await.ok();

        assert!(compressed.len() < data.len());
        assert_ne!(algorithm, CompressionAlgorithm::None);

        let decompressed = service.decompress(&compressed, algorithm).await.ok();
    ...
    async fn test_batching() {
        ...
        // 배치 디코딩
        let messages = MessageBatch::deserialize(&batch.unwrap_or_default()).unwrap_or_default();
