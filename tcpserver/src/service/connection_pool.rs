...
impl PooledConnection {
    ...
    pub fn is_expired(&self, config: &ConnectionPoolConfig) -> bool {
        ...
        self.request_count.load(Ordering::Relaxed) > config.max_requests_per_connection as u64
    }
...
impl AdvancedConnectionPool {
    ...
    pub async fn acquire(&self) -> Result<Arc<PooledConnection>> {
        ...
        self.stats.wait_time_us.fetch_add(wait_time.as_micros() as u64, Ordering::Relaxed);
        ...
        while let Some(conn_id) = idle_queue.pop_front() {
            ...
            if let Some(conn) = connections.get(&conn_id) {
                ...
                if !conn.is_expired(&self.config) && 
                   !conn.is_idle_timeout(&self.config).await {
    ...
    async fn create_connection(&self) -> Result<Arc<PooledConnection>> {
        ...
        self.stats.total_connections.store(connections.len(), Ordering::Relaxed);
...
mod tests {
    ...
    async fn test_connection_pool_creation() {
        ...
        let result = AdvancedConnectionPool::new(
            config,
            vec![addr],
            LoadBalancingStrategy::RoundRobin,
        ).await;
