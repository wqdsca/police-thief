...
impl RoomConnectionService {
    ...
    pub async fn add_user_to_room(
        ...
        self.room_info
            .entry(room_id)
            .or_insert_with(|| RoomInfo::new(room_id));
    ...
    pub async fn remove_user_from_room(&self, room_id: u32, user_id: u32) -> Result<()> {
        ...
        if user_removed {
            ...
            if let Some(redis_config) = &self.redis_config {
                ...
                tokio::spawn(async move {
                    if let Err(e) =
                        Self::remove_user_from_redis(redis_config, server_id, room_id, user_id)
                            .await
                    {
    ...
    pub async fn send_to_room(&self, room_id: u32, message: &GameMessage) -> Result<usize> {
        ...
        info!(
            "방 {} 메시지 전송 완료: 성공 {}, 실패 {}",
            room_id, success_count, failed_count
        );
        ...
        if let Some(from_room_id) = self.get_user_room(user_id) {
            ...
            if let Some(mut conn) = connection {
                ...
                if let Some(writer) = conn.writer.clone() {
                    ...
                    info!(
                        "사용자 {} 방 {}에서 방 {}로 이동 완료",
                        user_id, from_room_id, to_room_id
                    );
                } else {
                    return Err(anyhow!(
                        "사용자 {}의 연결 정보가 유효하지 않습니다",
                        user_id
                    ));
                }
            } else {
...
impl RoomConnectionService {
    ...
    async fn start_redis_sync(&self) -> Result<()> {
        ...
        let redis_config = self
            ...
            .as_ref()
            .map(|r| r.clone())
            .expect("Redis config must be set");
        let server_id = self.server_id.clone();
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) =
                    Self::sync_all_to_redis(&room_connections, &redis_config, &server_id).await
                {
    ...
    async fn sync_all_to_redis(
        ...
        for room_entry in room_connections.iter() {
            ...
            for (user_id, connection) in users.iter() {
                ...
                pipeline
                    .hset(
                        format!("tcp_server:{}:room:{}", server_id, room_id),
                        user_id,
                        &conn_json,
                    )
    ...
    async fn sync_user_to_redis(
        ...
        pipeline
            .hset(
                format!("tcp_server:{}:room:{}", server_id, connection.room_id),
                connection.user_id,
                &conn_json,
            )
            .sadd(
                format!("room:{}:users", connection.room_id),
                connection.user_id,
            )
            .hset(
                format!("user:{}:location", connection.user_id),
                "server_id",
                &server_id,
            )
            .hset(
    ...
    pub async fn restore_from_redis(&self) -> Result<usize> {
        if let Some(redis_config) = &self.redis_config {
            ...
            for key in keys {
                if let Ok(room_data) = conn
                    .hgetall::<String, HashMap<String, String>>(key.clone())
                    .await
                {
                    // 키에서 room_id 추출
                    if let Some(room_id_str) = key.split(':').next_back() {
