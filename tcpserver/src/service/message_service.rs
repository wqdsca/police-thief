...
pub type MessageHandler =
    Box<dyn Fn(u32, &GameMessage) -> Result<Option<GameMessage>> + Send + Sync>;
...
impl MessageService {
    ...
    fn get_message_type(message: &GameMessage) -> String {
        match message {
            ...
            GameMessage::Heartbeat { .. } => "heartbeat".to_string(),
            GameMessage::Echo { .. } => "echo".to_string(),
        }
    }
    ...
    pub async fn send_error(
        ...
        self.connection_service
            .send_to_user(client_id, &error_msg)
            .await?;

        Self::update_message_stats(&self.message_stats, "error", 0.0, true).await;
    ...
    pub async fn broadcast(&self, message: &GameMessage) -> Result<usize> {
        ...
        Self::update_message_stats(&self.message_stats, "broadcast", 0.0, true).await;
...
mod tests {
    ...
    async fn test_message_handler_registration() {
        ...
        // 테스트 핸들러 등록
        message_service
            .register_handler("test", |client_id, message| {
