...
use crate::protocol::{optimized::OptimizedGameMessage, GameMessage};
use crate::service::room_connection_service::{RoomConnectionService, RoomUserConnection};
...
impl ParallelBroadcastService {
    ...
    pub fn new(
        room_service: Arc<RoomConnectionService>,
        config: Option<ParallelBroadcastConfig>,
    ) -> Self {
        let config = config.unwrap_or_default();
        info!(
            "병렬 브로드캐스트 서비스 초기화 - 최대 작업: {}, 배치 크기: {}",
    ...
    pub async fn broadcast_to_room(&self, room_id: u32, message: &GameMessage) -> Result<usize> {
        ...
        // 통계 업데이트
        self.stats
            .record_broadcast(duration_us, successful as u64, failed as u64);
        self.stats
            .max_concurrent_connections
            .store(total_users as u64, Ordering::Relaxed);

        info!(
            "방 {} 브로드캐스트 완료: 성공 {}, 실패 {}, 시간 {}μs",
    ...
    pub async fn broadcast_to_multiple_rooms(
        ...
        let broadcast_futures: Vec<_> = room_ids
            ...
            .map(|&room_id| {
                ...
                task::spawn(async move { service.broadcast_to_room(room_id, &msg).await })
            })
            .collect();
        ...
        let duration_us = start_time.elapsed().as_micros() as u64;
        self.stats
            .parallel_rooms_processed
            .store(room_ids.len() as u64, Ordering::Relaxed);

        info!(
            "다중 방 브로드캐스트 완료: {}개 방, 총 {}명 성공, 시간 {}μs",
            room_ids.len(),
            total_successful,
            duration_us
        );
    ...
    /// 사용자들에게 병렬로 메시지 전송
    async fn parallel_send_to_users(
        &self,
        users: Vec<RoomUserConnection>,
        message_data: Vec<u8>,
    ) -> Result<Vec<Result<()>>> {
        ...
        let batch_futures: Vec<_> = batches
            ...
            .map(|batch| {
                ...
                task::spawn(async move { Self::send_batch_async(batch, data, timeout).await })
            })
            .collect();
    ...
    async fn send_batch_async(
        batch: Vec<RoomUserConnection>,
        message_data: Arc<Vec<u8>>,
        timeout_ms: u64,
    ) -> Vec<Result<()>> {
    ...
    pub fn get_stats(&self) -> ParallelBroadcastStats {
        ParallelBroadcastStats {
            ...
            successful_messages: AtomicU64::new(
                self.stats.successful_messages.load(Ordering::Relaxed),
            ),
            failed_messages: AtomicU64::new(self.stats.failed_messages.load(Ordering::Relaxed)),
            avg_broadcast_time_us: AtomicU64::new(
                self.stats.avg_broadcast_time_us.load(Ordering::Relaxed),
            ),
            parallel_rooms_processed: AtomicU64::new(
                self.stats.parallel_rooms_processed.load(Ordering::Relaxed),
            ),
            max_concurrent_connections: AtomicU64::new(
                self.stats
                    .max_concurrent_connections
...
impl PerformanceComparison {
    ...
    pub async fn compare_sequential_vs_parallel(
        ...
        for _ in 0..iterations {
            match sequential_service
                .send_to_room(room_id, &test_message)
                .await
            {
        ...
        let improvement_ratio =
            sequential_duration.as_millis() as f64 / parallel_result.total_duration_ms as f64;
