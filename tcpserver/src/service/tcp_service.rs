...
impl TcpGameService {
    ...
    pub fn new(config: TcpServerConfig) -> Self {
        ...
        let room_connection_service = Arc::new(RoomConnectionService::new("tcp_server".to_string()));
        let chat_room_handler = Arc::new(ChatRoomMessageHandler::new(room_connection_service.clone()));
        
        // 향상된 메모리 풀 초기화
        let enhanced_memory_pool = if config.enable_enhanced_memory_pool {
            let pool_config = config.memory_pool_config
                .clone()
                .unwrap_or_else(EnhancedPoolConfig::default);
            ...
            match listener.accept().await {
                Ok((stream, addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = Self::handle_client_connection(stream, addr.to_string(), chat_handler).await {
                            error!("클라이언트 처리 오류: {}", e);
                        }
                    });
                }
    ...
    async fn handle_client_connection(
        ...
        if !matches!(connect_message, GameMessage::Connect { .. }) {
            error!("첫 메시지가 Connect가 아님 ({}): {:?}", addr, connect_message);
            return Err(anyhow!("첫 메시지는 Connect 메시지여야 합니다"));
        }

        info!("Connect 메시지 수신 완료: {} -> {:?}", addr, connect_message);
    ...
    pub async fn schedule_async_task<F>(&self, task: F, priority: TaskPriority) -> Result<(), &'static str>
    where
    ...
    {
        if let Some(scheduler) = &self.async_scheduler {
            scheduler.schedule_with_deadline(task, priority, deadline).await
        } else {
            // 스케줄러가 비활성화된 경우 직접 실행
            tokio::spawn(task);
            Ok(())
    ...
    pub async fn schedule_message_processing<F>(&self, task: F, is_critical: bool) -> Result<(), &'static str>
    where
        F: std::future::Future<Output = ()> + Send + 'static,
    {
...
mod tests {
    ...
    async fn test_async_scheduler() {
        ...
        // 비동기 작업 스케줄링 테스트
        let result = service.schedule_async_task(async {
        ...
        // 메시지 처리 스케줄링
        let result = service.schedule_message_processing(async {
            // 모의 메시지 처리
        }, true).await;
        ...
        // 백그라운드 정리 작업 스케줄링
        let result = service.schedule_background_cleanup(async {
            // 모의 정리 작업
        }).await;
