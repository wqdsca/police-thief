...
impl HeartbeatService {
                ...
                if let Ok(mut stats) = stats_ref.try_lock() {
                    ...
                    if stats.average_response_time_ms == 0.0 {
                        ...
                        stats.average_response_time_ms =
                            (stats.average_response_time_ms * 0.9) + (cleanup_time_ms * 0.1);
                    }
                }

                if cleanup_count > 0 {
                    info!(
                        "하트비트 타임아웃 연결 정리: {}개 (활성: {}개)",
                        cleanup_count, current_connections
                    );
    ...
    /// Update heartbeat for a user (placeholder)
    pub async fn update_heartbeat(&self, user_id: u32) {
        debug!("Heartbeat updated for user {}", user_id);
    ...
    pub async fn handle_heartbeat(&self, client_id: u32) -> Result<()> {
        ...
        if let Err(e) = self
            .connection_service
            .send_to_user(client_id, &response)
            .await
        {
            let tcp_error =
                TcpServerError::heartbeat_error(Some(client_id), "send_response", &e.to_string());
