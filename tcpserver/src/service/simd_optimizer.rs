...
impl SimdOptimizer {
    ...
    pub fn simd_memcmp(&self, a: &[u8], b: &[u8]) -> bool {
        ...
        self.stats.bytes_processed.fetch_add(len as u64, Ordering::Relaxed);
        ...
        #[cfg(target_arch = "x86_64")]
        {
            if self.config.enable_avx2 && self.capabilities.has_avx2 {
                ...
                self.stats.vectorized_operations.fetch_add(1, Ordering::Relaxed);
    ...
    pub fn simd_xor(&self, a: &[u8], b: &[u8]) -> Vec<u8> {
        ...
        self.stats.total_operations.fetch_add(1, Ordering::Relaxed);
        self.stats.bytes_processed.fetch_add(len as u64 * 2, Ordering::Relaxed);
    ...
    pub fn allocate_aligned(&self, size: usize, alignment: usize) -> Vec<u8> {
        ...
        self.stats.alignment_optimizations.fetch_add(1, Ordering::Relaxed);
        ...
        F: Fn(&[u8]) -> Vec<u8> + Send + Sync + 'static,
    {
        ...
        self.stats.parallel_executions.fetch_add(1, Ordering::Relaxed);
...
impl SimdPerformanceStats {
    ...
    pub fn performance_score(&self) -> f64 {
        ...
        let parallel_score = if self.parallel_executions > 0 { 15.0 } else { 0.0 };
        let alignment_score = if self.alignment_optimizations > 0 { 15.0 } else { 0.0 };
...
mod tests {
    ...
    async fn test_parallel_simd() {
        ...
        let results = optimizer.parallel_simd_process(data, |d| {
            d.iter().map(|&x| x * 2).collect()
        }).await;
