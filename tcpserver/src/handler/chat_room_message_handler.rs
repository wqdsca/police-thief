...
impl ChatRoomMessageHandler {
    ...
    pub async fn handle_client_connection(
        ...
        // Connect 메시지 처리
        let (user_id, _room_id) = match connect_message {
            GameMessage::Connect { room_id, user_id } => {
                info!(
                    "새 클라이언트 연결: user_id={}, room_id={}, addr={}",
    ...
    async fn message_loop(
        ...
        loop {
            ...
            // 메시지 타입별 처리
            if let Err(e) = self
                .route_message(user_id, &addr, writer.clone(), message)
                .await
            {
    ...
    async fn route_message(
        ...
        match message {
            // 하트비트 처리
            GameMessage::HeartBeat => self.handle_heartbeat(user_id, writer).await,
            ...
            GameMessage::RoomJoin {
                ...
                self.handle_room_join(user_id, room_id, nickname, addr.to_string(), writer)
                    .await
            }

            // 방 퇴장 처리
            GameMessage::RoomLeave {
                user_id: msg_user_id,
                room_id,
            } => {
                if msg_user_id != user_id {
    ...
    async fn handle_room_join(
        ...
        match self
            .chat_handler
            .handle_room_join(user_id, room_id, nickname, addr, writer)
            .await
        {
            Ok(user_count) => {
                info!(
                    "✅ 사용자 {} 방 {} 입장 성공, 현재 인원: {}",
                    user_id, room_id, user_count
                );
                Ok(())
            }
            Err(e) => {
    ...
    async fn handle_chat_message(&self, user_id: u32, room_id: u32, content: String) -> Result<()> {
        ...
        match self
            .chat_handler
            .handle_chat_message(user_id, room_id, content.clone())
            .await
        {
            Ok(sent_count) => {
                info!(
