...
impl ChatRoomHandler {
    ...
    pub async fn handle_room_join(
        ...
        if let Some(current_room) = self.room_service.get_user_room(user_id) {
            if current_room != room_id {
                warn!("사용자 {}가 방 {}에서 방 {}로 이동", user_id, current_room, room_id);
        ...
        // 방에 사용자 추가
        self.room_service
            .add_user_to_room(room_id, user_id, addr.clone(), nickname.clone(), writer.clone())
            .await?;

        let user_count = self.room_service.get_room_user_count(room_id);

        // 입장한 사용자에게 성공 메시지 전송
        let join_success = GameMessage::RoomJoinSuccess { room_id, user_count };
    ...
    pub async fn handle_room_leave(&self, user_id: u32, room_id: u32) -> Result<bool> {
        ...
        let leaving_user = leaving_user.unwrap();

        // 방에서 사용자 제거
        self.room_service.remove_user_from_room(room_id, user_id).await?;

        let new_user_count = self.room_service.get_room_user_count(room_id);

        // 퇴장한 사용자에게 퇴장 성공 메시지 전송
        let leave_success = GameMessage::RoomLeaveSuccess { room_id, user_count: new_user_count };
    ...
    pub async fn handle_chat_message(
        ...
        let sent_count = self.room_service.send_to_room(room_id, &chat_message).await?;
    ...
    pub async fn handle_user_disconnect(&self, user_id: u32) -> Result<usize> {
        ...
        info!("✅ 사용자 {} 연결 해제 처리 완료, 정리된 방: {}", user_id, cleaned_rooms);
        Ok(cleaned_rooms)
    }
