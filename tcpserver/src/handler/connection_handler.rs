...
use crate::tool::{ConnectionQuality, IpInfo, NetworkUtils};
...
impl ConnectionHandler {
    ...
    pub async fn handle_new_connection(&self, stream: TcpStream, addr: String) -> Result<u32> {
        ...
        let (room_id, user_id) = match connect_msg {
            GameMessage::Connect { room_id, user_id } => {
                info!(
                    "Connect 메시지 수신: room_id={}, user_id={}",
                    room_id, user_id
                );
                (room_id, user_id)
            }
        ...
        if let Some(redis_config) = &self.redis_config {
            if let Err(e) = self
                .store_tcp_host_to_redis(user_id, &addr, redis_config.as_ref())
                .await
            {
        ...
        let registered_user_id = self
            .connection_service
            .handle_new_connection_with_id(reunited_stream, addr.clone(), user_id)
            .await?;
    ...
    async fn validate_user_connection(&self, ip_info: &IpInfo) -> Result<()> {
        ...
        let _connection_stats = self.connection_service.get_connection_stats().await;
    ...
    /// TCP 호스트 정보를 Redis에 저장
    async fn store_tcp_host_to_redis(
        &self,
        user_id: u32,
        addr: &str,
        ...
        let _: () = conn
            .hset(&user_key, "tcp_host", addr)
            .await
            .map_err(|e| anyhow!("Redis HSET 실패: {}", e))?;

        // TTL 갱신 (1시간)
        let _: () = conn
            .expire(&user_key, 3600)
            .await
            .map_err(|e| anyhow!("Redis EXPIRE 실패: {}", e))?;

        debug!(
            "사용자 {} TCP 호스트 정보 Redis 저장 완료: {}",
            user_id, addr
        );
        Ok(())
    }
    ...
    async fn send_welcome_message(&self, user_id: u32) -> Result<()> {
        ...
        self.connection_service
            .send_to_user(user_id, &welcome_message)
            .await?;
    ...
    pub async fn identify_problematic_connections(&self) -> Vec<ProblematicConnection> {
        ...
        for user in users {
            ...
            if last_heartbeat_secs > 1500 {
                // 25분 - 타임아웃 임박 (30분 타임아웃 전 5분)
                problematic.push(ProblematicConnection {
                    user_id: user.user_id,
                    addr: user.addr,
                    issue: "하트비트 지연".to_string(),
                    severity: if last_heartbeat_secs > 1800 {
                        "높음"
                    } else {
                        "보통"
                    }
...
mod tests {
    ...
    async fn test_connection_handler() {
        ...
        let handler =
            ConnectionHandler::new(connection_service, heartbeat_service, message_service);
