...
async fn basic_logging_example() -> Result<()> {
    ...
    // 다양한 로그 레벨로 메시지 작성
    logger
        .trace("상세한 추적 정보", &[("function", "basic_example")])
        .await;
    logger
        .debug("디버깅 정보", &[("step", "1"), ("status", "processing")])
        .await;
    logger
        .info(
            "일반 정보",
            &[("action", "server_start"), ("port", "50051")],
        )
        .await;
    logger
        .warn(
    ...
    logger
        .error(
            "오류 발생",
            &[("error_code", "E001"), ("component", "database")],
        )
        .await;
    logger
        .fatal("심각한 오류", &[("reason", "system_failure")])
        .await;
...
async fn custom_config_example() -> Result<()> {
    ...
    logger
        .info(
            "서버 바인딩 성공",
            &[("address", "0.0.0.0:4000"), ("protocol", "tcp")],
        )
        .await;
    ...
    logger
        .info(
            "플레이어 풀 초기화",
            &[("initial_capacity", "1000"), ("max_connections", "5000")],
        )
        .await;
...
async fn game_server_scenario() -> Result<()> {
    ...
    for i in 1..=5 {
        ...
        logger
            .debug(
                "플레이어 인증 시작",
                &[("player_id", &player_id), ("auth_method", "jwt")],
            )
            .await;
...
async fn error_handling_example() -> Result<()> {
    ...
    logger
        .info(
            "게임 센터 서비스 시작",
            &[("service", "game_center"), ("version", "2.1.0")],
        )
        .await;
    ...
    logger
        .info(
            "시스템 복구 시작",
            &[("recovery_type", "automatic"), ("estimated_time_sec", "30")],
        )
        .await;
...
async fn performance_example() -> Result<()> {
    ...
    logger
        .info(
            "성능 테스트 시작",
            &[("test_type", "high_throughput"), ("target_logs", "1000")],
        )
        .await;
    ...
    logger
        .info(
            ...
            &[
                ...
                (
                    "logs_per_second",
                    &(1000.0 / duration.as_secs_f64()).to_string(),
                ),
            ],
        )
        .await;
