...
impl InputValidator {
    pub fn new() -> Self {
        ...
        // Username 규칙
        rules.insert(InputType::Username, ValidationRule {
            min_length: Some(3),
            max_length: Some(32),
        ...
        // Email 규칙
        rules.insert(InputType::Email, ValidationRule {
            min_length: Some(5),
            max_length: Some(254),
        ...
        // Password 규칙
        rules.insert(InputType::Password, ValidationRule {
            min_length: Some(8),
            max_length: Some(128),
        ...
        // Room Name 규칙
        rules.insert(InputType::RoomName, ValidationRule {
            min_length: Some(1),
            max_length: Some(50),
        ...
        // Chat Message 규칙
        rules.insert(InputType::ChatMessage, ValidationRule {
            min_length: Some(1),
            max_length: Some(500),
            pattern: None,
            required: true,
        ...
        // Text 규칙
        rules.insert(InputType::Text, ValidationRule {
            min_length: Some(0),
            max_length: Some(10000),
            pattern: None,
            required: false,
    ...
    pub fn validate(&self, input: &str, input_type: InputType) -> Result<String, String> {
        ...
        if input.len() > self.max_input_size {
            return Err(format!("Input too large: {} bytes (max: {})", input.len(), self.max_input_size));
        }
        
        // 규칙 가져오기
        let rule = self.rules.get(&input_type)
            .ok_or_else(|| format!("No validation rule for {:?}", input_type))?;
        
        // 필수 필드 체크
        if rule.required && input.is_empty() {
            return Err(format!("{:?} is required", input_type));
        }
        
        // 길이 체크
        if let Some(min) = rule.min_length {
            if input.len() < min {
                return Err(format!("{:?} must be at least {} characters", input_type, min));
            }
        }
        
        if let Some(max) = rule.max_length {
            if input.len() > max {
                return Err(format!("{:?} must be at most {} characters", input_type, max));
            }
        }
        ...
        if let Some(ref pattern) = rule.pattern {
            if !pattern.is_match(input) {
                return Err(format!("{:?} has invalid format", input_type));
            }
        }
    ...
    pub fn validate_file_path(&self, input: &str) -> Result<String, String> {
        ...
        // 허용된 문자만 포함하는지 체크
        let safe_path_pattern = Regex::new(r#"^[a-zA-Z0-9\-_./ \\]+$"#).unwrap();
...
mod tests {
    ...
    fn test_username_validation() {
        ...
        assert!(validator.validate("user@name", InputType::Username).is_err()); // Invalid char
    }
    ...
    fn test_email_validation() {
        ...
        assert!(validator.validate("@example.com", InputType::Email).is_err());
    }
    ...
    fn test_password_strength() {
        ...
        assert_eq!(validator.check_password_strength("weak"), PasswordStrength::Weak);
        assert_eq!(validator.check_password_strength("Password1"), PasswordStrength::Fair);
        assert_eq!(validator.check_password_strength("P@ssw0rd123"), PasswordStrength::Good);
