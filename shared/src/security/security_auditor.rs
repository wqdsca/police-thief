...
use crate::security::{AccessControlMatrix, RateLimiter, RedisCommandValidator, SecurityConfig};
use anyhow::Result;
...
impl SecurityAuditor {
    ...
    pub async fn run_audit(&mut self) -> Result<AuditResult> {
        ...
        if matches!(
            self.audit_level,
            AuditLevel::Comprehensive | AuditLevel::Full
        ) {
    ...
    async fn check_jwt_secret_security(&mut self) {
        ...
        match env::var("JWT_SECRET_KEY") {
            Ok(secret) => {
                ...
                let weak_patterns = [
                    "default", "secret", "change", "your_", "please", "example", "insecure",
                    "test", "demo",
                ];
                ...
                for pattern in weak_patterns {
                    if lower_secret.contains(pattern) {
                        self.add_issue(SecurityIssue {
                            ...
                            description: format!(
                                "JWT 시크릿 키에 '{pattern}' 패턴이 포함되어 있습니다."
                            ),
                ...
                if entropy < 4.2 {
                    self.add_issue(SecurityIssue {
                        ...
                        description: format!(
                            "JWT 시크릿 키의 엔트로피가 낮습니다 ({entropy}). 더 랜덤한 키가 필요합니다."
                        ),
            ...
            if let Ok(rounds_num) = rounds.parse::<u32>() {
                if rounds_num < 10 {
                    self.add_issue(SecurityIssue {
                        ...
                        description: format!(
                            "BCrypt 라운드가 {rounds_num}입니다. 최소 10 이상 권장."
                        ),
                        severity: Severity::Medium,
    ...
    async fn validate_security_config(&mut self, config: &SecurityConfig) {
        ...
        if config.jwt_expiration_hours > 24 {
            self.add_issue(SecurityIssue {
                ...
                description: format!(
                    "JWT 토큰이 {}시간 동안 유효합니다. 보안을 위해 더 짧게 설정하세요.",
        ...
        if config.rate_limit_rpm > 120 {
            self.add_issue(SecurityIssue {
                ...
                description: format!(
                    "분당 {}개 요청을 허용합니다. DDoS 보호를 위해 더 엄격하게 설정하세요.",
        ...
        if config.max_message_size > 1024 * 1024 {
            self.add_issue(SecurityIssue {
                ...
                description: format!(
                    "최대 메시지 크기가 {}바이트입니다. DoS 공격 방지를 위해 제한하세요.",
        ...
        if config.bcrypt_rounds < 10 || config.bcrypt_rounds > 15 {
            self.add_issue(SecurityIssue {
                ...
                description: format!(
                    "BCrypt 라운드가 {}입니다. 10-15 사이가 권장됩니다.",
    ...
    async fn audit_jwt_configuration(&mut self) -> Result<()> {
        ...
        if !matches!(algorithm.as_str(), "HS256" | "HS384" | "HS512") {
            self.add_issue(SecurityIssue {
                ...
                description: format!("JWT 알고리즘 '{algorithm}'는 지원되지 않습니다."),
                severity: Severity::High,
                category: "인증".to_string(),
        ...
        if refresh_days > 30 {
            self.add_issue(SecurityIssue {
                ...
                description: format!("Refresh 토큰이 {refresh_days}일 동안 유효합니다."),
                severity: Severity::Low,
                category: "인증".to_string(),
    ...
    async fn audit_rate_limiting(&mut self) -> Result<()> {
        ...
        let stats = RateLimiter::default().get_stats().await;
    ...
    async fn audit_data_protection(&mut self) -> Result<()> {
        ...
        if let Ok(log_content) =
            fs::read_to_string("server.log").or_else(|_| fs::read_to_string("app.log"))
        {
        ...
        let backup_encryption =
            env::var("BACKUP_ENCRYPTION_ENABLED").unwrap_or_else(|_| "false".to_string());
    ...
    async fn audit_redis_security(&mut self) -> Result<()> {
        ...
        match RedisCommandValidator::with_default_config() {
            Ok(validator) => {
                // 기본 명령어 검증 테스트
            ...
            #[cfg(windows)]
            {
                ...
                if jwt_secret.len() >= 64 && redis_pass.len() >= 16 && db_pass.len() >= 16 {
                    ...
                    self.add_issue(SecurityIssue {
                        ...
                        remediation: "모든 패스워드를 16자 이상, JWT 키를 64자 이상으로 설정"
                            .to_string(),
        ...
        if let Ok(cargo_content) = fs::read_to_string("Cargo.toml") {
            ...
            if cargo_content.contains("openssl") {
                self.add_issue(SecurityIssue {
                    ...
                    description: "OpenSSL에 의존하고 있습니다. 정기적인 업데이트가 필요합니다."
                        .to_string(),
                    severity: Severity::Low,
    ...
    async fn audit_performance_security(&mut self) -> Result<()> {
        ...
        if max_connections > 10000 {
            self.add_issue(SecurityIssue {
                ...
                description: format!("최대 연결 수가 {max_connections}개로 설정되어 있습니다."),
                severity: Severity::Low,
        ...
        if request_timeout > 120 {
            self.add_issue(SecurityIssue {
                ...
                description: format!("요청 타임아웃이 {request_timeout}초로 설정되어 있습니다."),
                severity: Severity::Low,
                category: "성능".to_string(),
    ...
    async fn generate_result(&self, duration: Duration) -> Result<AuditResult> {
        ...
        system_info.insert(
            ...
            (self.check_count - self.passed_count).to_string(),
        );
        ...
        if critical_count > 0 {
            recommendations.push(format!(
                "🚨 {critical_count}개의 치명적 이슈를 즉시 해결하세요."
            ));
        }

        if high_count > 0 {
            recommendations.push(format!(
                "⚠️ {high_count}개의 높은 위험 이슈를 24시간 내에 해결하세요."
            ));
        }
        ...
        if total_score >= 95 {
            ...
            recommendations
                .push("✅ 프로덕션 배포 준비 완료. 몇 가지 개선사항을 검토하세요.".to_string());
...
mod tests {
    ...
    async fn test_basic_audit() {
        ...
        let result = auditor.run_audit().await.expect("Test assertion failed");
