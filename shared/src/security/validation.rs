...
impl InputValidator {
    ...
    pub fn new(config: SecurityConfig) -> Result<Self, SecurityError> {
        let sql_injection_patterns = vec![
            ...
            Regex::new(r"(?i)(update\s+set)").unwrap(),
        ...
        let dangerous_patterns = vec![
            ...
            Regex::new(r"(?i)(cmd|powershell|bash|sh)\s").unwrap(), // Command injection
            Regex::new(r#"[<>"'&]"#).unwrap(), // HTML/XML 특수문자
        ];
    ...
    /// 길이 검증 헬퍼 함수
    fn validate_length(&self, input: &str, field_name: &str, min: usize, max: usize) -> Result<(), SecurityError> {
    ...
    /// 종합 사용자 입력 검증
    pub fn validate_user_input(&self, input: &HashMap<String, String>) -> Result<(), SecurityError> {
        for (key, value) in input {
            ...
            match key.as_str() {
                ...
                _ => {
                    ...
                    if value.len() > 1000 {
                        return Err(SecurityError::InvalidInput(
                            format!("Field '{}' is too long (max 1000 characters)", key)
                        ));
                    }
                }
            }
