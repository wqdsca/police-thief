...
impl JwtManager {
    ...
    pub async fn verify_token(&self, token: &str) -> Result<Claims, SecurityError> {
        ...
        if self.is_blacklisted(token).await {
            return Err(SecurityError::InvalidToken("Token is blacklisted".to_string()));
        }
    ...
    pub async fn refresh_access_token(&self, refresh_token: &str) -> Result<String, SecurityError> {
        ...
        // 여기서는 기본값 사용
        self.create_access_token(&refresh_claims.sub, "user", vec!["user".to_string()]).await
    }
    ...
    pub async fn blacklist_token(&self, token: &str) -> Result<(), SecurityError> {
        // JWT ID 추출
        let token_data = decode::<Claims>(token, &self.decoding_key, &self.validation)
            .map_err(|e| SecurityError::InvalidToken(format!("Cannot blacklist invalid token: {}", e)))?;
    ...
    pub async fn verify_admin_access(&self, token: &str) -> Result<Claims, SecurityError> {
        ...
        if !claims.roles.contains(&"admin".to_string()) {
            return Err(SecurityError::AuthorizationDenied("Admin access required".to_string()));
        }
...
mod tests {
    ...
    async fn test_jwt_creation_and_verification() {
        ...
        // 토큰 생성
        let token = jwt_manager.create_access_token(
            "user123",
            "testuser",
            vec!["user".to_string()]
        ).await.unwrap();
    ...
    async fn test_token_blacklist() {
        ...
        // 블랙리스트 추가
        jwt_manager.blacklist_token(&token).await.unwrap();
