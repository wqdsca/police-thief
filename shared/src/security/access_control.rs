...
pub struct AccessControlMatrix {
    ...
    /// 특별한 권한 규칙
    custom_rules: Vec<Box<dyn Fn(&UserRole, &ApiEndpoint, Option<i32>) -> bool + Send + Sync>>,
}
...
impl AccessControlMatrix {
    ...
    fn initialize_default_permissions(&mut self) {
        ...
        self.role_permissions.insert(UserRole::Premium, premium_perms);
        ...
        self.role_permissions.insert(UserRole::GameMaster, gamemaster_perms);
        ...
        self.role_permissions.insert(UserRole::Moderator, moderator_perms);
        ...
        self.role_permissions.insert(UserRole::SuperAdmin, superadmin_perms);
    }
    ...
    fn initialize_public_endpoints(&mut self) {
        self.public_endpoints.insert(ApiEndpoint::new("user", "LoginUser"));
        self.public_endpoints.insert(ApiEndpoint::new("user", "RegisterUser"));
        self.public_endpoints.insert(ApiEndpoint::new("health", "HealthCheck"));
        self.public_endpoints.insert(ApiEndpoint::new("info", "GetServerInfo"));
    }
    ...
    pub fn check_permission(
        ...
        let required_permissions = match self.endpoint_permissions.get(endpoint) {
            ...
            None => {
                ...
                return Err(format!("정의되지 않은 엔드포인트: {}", endpoint.full_path()));
            }
        };
        ...
        if !has_all_permissions {
            let missing_perms: Vec<&Permission> = required_permissions
    ...
    pub fn get_minimum_role_for_endpoint(&self, endpoint: &ApiEndpoint) -> Option<UserRole> {
        ...
        for role in roles.iter() {
            if let Some(role_perms) = self.role_permissions.get(role) {
                if required_permissions.iter().all(|perm| role_perms.contains(perm)) {
    ...
    pub fn validate_matrix(&self) -> Vec<String> {
        ...
        for role in [
            ...
            UserRole::GameMaster, UserRole::Moderator, 
            UserRole::Admin, UserRole::SuperAdmin
        ] {
            if !self.role_permissions.contains_key(&role) {
                issues.push(format!("역할 {:?}에 권한이 정의되지 않았습니다", role));
            }
        }
        ...
        for (higher, lower) in roles {
            if let (Some(higher_perms), Some(lower_perms)) = 
                (self.role_permissions.get(&higher), self.role_permissions.get(&lower)) {
                
                let missing_perms: Vec<&Permission> = lower_perms.difference(higher_perms).collect();
                if !missing_perms.is_empty() {
                    issues.push(format!(
                        "역할 {:?}가 하위 역할 {:?}의 권한을 상속하지 않습니다: {:?}",
                        higher, lower, missing_perms
                    ));
                }
            }
        }
    ...
    pub fn get_statistics(&self) -> HashMap<String, usize> {
        ...
        stats.insert("총_엔드포인트_수".to_string(), 
            self.endpoint_permissions.len() + self.public_endpoints.len());
        stats.insert("보호된_엔드포인트_수".to_string(), self.endpoint_permissions.len());
        stats.insert("공개_엔드포인트_수".to_string(), self.public_endpoints.len());
...
mod tests {
    ...
    fn test_role_from_string() {
        ...
        assert_eq!(UserRole::from_str("game_master"), Some(UserRole::GameMaster));
    ...
    fn test_protected_endpoints() {
        ...
        // 권한이 없으면 접근 불가
        assert!(matrix.check_permission(&[UserRole::Guest], &create_room_endpoint, None).is_err());
        
        // 적절한 권한이 있으면 접근 가능
        assert!(matrix.check_permission(&[UserRole::User], &create_room_endpoint, None).is_ok());
    }
    ...
    fn test_admin_endpoints() {
        ...
        assert!(matrix.check_permission(&[UserRole::User], &admin_endpoint, None).is_err());
        
        // 관리자는 접근 가능
        assert!(matrix.check_permission(&[UserRole::Admin], &admin_endpoint, None).is_ok());
    }
    ...
    fn test_multiple_roles() {
        ...
        assert!(matrix.check_permission(&roles, &ban_user_endpoint, None).is_ok());
    }
    ...
    fn test_minimum_role_calculation() {
        ...
        assert_eq!(matrix.get_minimum_role_for_endpoint(&create_room), Some(UserRole::User));
        assert_eq!(matrix.get_minimum_role_for_endpoint(&system_config), Some(UserRole::Admin));
    }
    ...
    fn test_statistics() {
        ...
        assert!(stats.get("총_역할_수").unwrap() > &0);
        assert!(stats.get("총_엔드포인트_수").unwrap() > &0);
        assert!(stats.get("고유_권한_수").unwrap() > &0);
