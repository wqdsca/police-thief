...
impl SecurityMiddleware {
    ...
    pub async fn authenticate(
        &self,
        token: &str,
    ) -> Result<crate::security::Claims, SecurityError> {
    ...
    pub async fn register_user(
        ...
        // 입력 검증
        self.input_validator
            .validate(username, crate::security::InputType::Username)
            .map_err(SecurityError::InvalidInput)?;
        self.input_validator
            .validate(password, crate::security::InputType::Password)
            .map_err(SecurityError::InvalidInput)?;
        self.input_validator
            .validate(email, crate::security::InputType::Email)
            .map_err(SecurityError::InvalidInput)?;
...
mod tests {
    ...
    async fn test_security_middleware() {
        ...
        let middleware = SecurityMiddleware::new(config).await.expect("Failed to create security middleware");

        // Rate limiting 테스트
        let test_ip = IpAddr::from_str("192.168.1.100").expect("Failed to parse IP address");
        assert!(middleware.check_rate_limit(test_ip).await.is_ok());
    ...
    async fn test_user_registration() {
        ...
        // 유효한 등록
        assert!(middleware
            .register_user("testuser", "SecurePass123!", "test@example.com")
            .await
            .is_ok());

        // 무효한 등록 (약한 비밀번호)
        assert!(middleware
            .register_user("testuser", "weak", "test@example.com")
            .await
            .is_err());
    }
}
