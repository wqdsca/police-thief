...
impl RateLimiter {
    ...
    /// 특정 IP 수동 차단
    pub async fn block_ip(&self, ip: IpAddr, duration_minutes: Option<u64>) -> Result<(), SecurityError> {
        let now = Instant::now();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                entries.retain(|_ip, entry| {
                    ...
                    let should_remove = now.duration_since(entry.last_request) > Duration::from_secs(3600);
                ...
                if removed_count > 0 {
                    ...
                    stats.current_blocked_ips = stats.current_blocked_ips.saturating_sub(unblocked_count);
...
mod tests {
    ...
    async fn test_rate_limiting() {
        ...
        // 처음 5개 요청은 허용
        for _ in 0..5 {
            assert!(limiter.is_allowed(test_ip).await.unwrap());
        }
    ...
    async fn test_manual_blocking() {
        ...
        let test_ip = IpAddr::from_str("10.0.0.1").unwrap();
        
        // 처음에는 허용
        assert!(limiter.is_allowed(test_ip).await.unwrap());
        
        // 수동 차단
        limiter.block_ip(test_ip, Some(1)).await.unwrap();
        
        // 차단 후에는 거부
        assert!(!limiter.is_allowed(test_ip).await.unwrap());
        
        // 수동 해제
        limiter.unblock_ip(test_ip).await.unwrap();
