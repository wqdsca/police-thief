...
impl RedisCommandValidator {
        ...
        if self.config.enable_dangerous_pattern_detection {
            for pattern in &self.dangerous_patterns {
                if pattern.is_match(key) {
                    ...
                    return Err(anyhow::anyhow!("위험 패턴이 감지된 Redis 키: {}", key));
                }
            }
        }
    ...
    pub fn validate_operation(
        ...
        // 키 검증
        self.validate_key(key).context("Redis 키 검증 실패")?;

        // 필드 검증 (해시 명령어의 경우)
        if let Some(f) = field {
            self.validate_field(f).context("Redis 필드 검증 실패")?;
        }

        // 값 검증
        if let Some(v) = value {
            self.validate_value(v).context("Redis 값 검증 실패")?;
        }
...
mod tests {
    ...
    fn test_allowed_commands() {
        let validator = RedisCommandValidator::default().expect("Test assertion failed");
    ...
    fn test_dangerous_patterns() {
        ...
        // SQL 주입 패턴
        assert!(validator
            .validate_key("user'; DROP TABLE users; --")
            .is_err());
        assert!(validator.validate_field("field UNION SELECT").is_err());

        // 스크립트 주입 패턴
        assert!(validator
            .validate_key("<script>alert('xss')</script>")
            .is_err());
        ...
        assert!(validator
            .validate_key("\\windows\\system32\\config")
            .is_err());
    }
    ...
    fn test_value_validation() {
        ...
        assert!(validator
            .validate_value(b"{\"user_id\":123,\"score\":456}")
            .is_ok());
    ...
    fn test_comprehensive_operation() {
        ...
        // 정상적인 HSET 연산
        assert!(validator
            .validate_operation("HSET", "user:123", Some("username"), Some(b"john_doe"))
            .is_ok());

        // 비정상적인 연산 - 위험한 명령어
        assert!(validator
            .validate_operation("EVAL", "user:123", None, Some(b"redis.call('FLUSHALL')"))
            .is_err());

        // 비정상적인 연산 - 위험한 키
        assert!(validator
            .validate_operation("GET", "../../../etc/passwd", None, None)
            .is_err());
    }
