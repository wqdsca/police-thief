...
use shared::security::{SecurityAuditor, AuditLevel};
use anyhow::Result;
use std::env;
...
async fn main() -> Result<()> {
    // 환경변수 설정 (테스트용)
    env::set_var("JWT_SECRET_KEY", "super_secure_production_key_with_32_plus_characters_for_maximum_security_2024");
    ...
    match auditor.run_audit().await {
        Ok(result) => {
            println!("\n📊 보안 감사 결과:");
            println!("================");
            println!("🎯 총 점수: {}/100 ({})", result.total_score, result.get_grade());
            println!("⏱️ 감사 시간: {}ms", result.duration_ms);
            println!("✅ 통과한 검사: {}/{}", result.passed_checks, result.total_checks);
            ...
            if result.issues.is_empty() {
                ...
                for issue in &result.issues {
                    println!("  {} {} [{}]", issue.severity.emoji(), issue.title, issue.category);
                    if matches!(issue.severity, shared::security::Severity::Critical | shared::security::Severity::High) {
            ...
            if result.total_score == 100 {
                ...
                println!("\n⚡ 현재 {}점 - 추가 보안 강화가 필요합니다.", result.total_score);
            }
        }
        Err(e) => {
