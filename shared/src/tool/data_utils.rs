...
impl DataUtils {
    ...
    pub fn deserialize_with_length<T: for<'de> Deserialize<'de>>(data: &[u8]) -> Result<T> {
        ...
        if data.len() < 4 + length {
            return Err(anyhow!("메시지 길이 불일치: {} < {}", data.len(), 4 + length));
        }
    ...
    pub async fn read_exact_data<R: AsyncReadExt + Unpin>(stream: &mut R, length: usize) -> Result<Vec<u8>> {
    ...
    /// 스트림에 데이터 전송 (플러시 포함)
    pub async fn write_data_flush<W: AsyncWriteExt + Unpin>(stream: &mut W, data: &[u8]) -> Result<TransferResult> {
...
mod tests {
    ...
    fn test_serialize_deserialize_with_length() {
        ...
        let serialized = DataUtils::serialize_with_length(&test_data).unwrap();
        let deserialized: TestData = DataUtils::deserialize_with_length(&serialized).unwrap();
