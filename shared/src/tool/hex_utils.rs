...
impl HexUtils {
    ...
    pub fn hex_to_u32(hex: &str) -> Result<u32> {
        ...
        u32::from_str_radix(&hex, 16).map_err(|e| anyhow!("u32 변환 실패: {} ({})", hex, e))
    }

    /// 16진수 문자열을 u64로 변환
    pub fn hex_to_u64(hex: &str) -> Result<u64> {
        let hex = hex.trim().replace("0x", "");
        u64::from_str_radix(&hex, 16).map_err(|e| anyhow!("u64 변환 실패: {} ({})", hex, e))
    }
...
mod tests {
    ...
    fn test_bytes_hex_conversion() {
        ...
        let converted_back = HexUtils::hex_to_bytes(&hex).expect("Test assertion failed");
    ...
    fn test_hex_with_prefix() {
        ...
        let bytes = HexUtils::hex_to_bytes(hex_with_prefix).expect("Test assertion failed");
    ...
    fn test_u32_conversion() {
        ...
        let converted_back = HexUtils::hex_to_u32(&hex).expect("Test assertion failed");
    ...
    fn test_extract_u32() {
        ...
        let be_value = HexUtils::extract_u32_be(&bytes, 0).expect("Test assertion failed");
        let le_value = HexUtils::extract_u32_le(&bytes, 0).expect("Test assertion failed");
