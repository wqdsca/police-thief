...
impl BlockingTaskExecutor {
    ...
    pub fn new(pool_size: Option<usize>) -> Self {
        ...
        info!(
            "BlockingTaskExecutor initialized with {} threads",
            pool_size
        );
...
macro_rules! cpu_intensive {
    ($name:expr, $body:expr) => {
        $crate::tool::high_performance::blocking_task_executor::execute_blocking($name, move || {
            $body
        })
        .await
    };
...
mod tests {
    ...
    async fn test_multiple_tasks() {
        ...
        for (i, handle) in handles.into_iter().enumerate() {
            let result = handle.await.ok().unwrap_or_default();
    ...
    async fn test_global_executor() {
        let result = execute_blocking("global_test", || 42).await;
