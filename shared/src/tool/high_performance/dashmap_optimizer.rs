...
impl ShardLoadStats {
    pub fn new(shard_count: usize) -> Self {
        Self {
            ...
            reads_per_shard: (0..shard_count)
                .map(|_| CacheAlignedCounter::new())
                .collect(),
            writes_per_shard: (0..shard_count)
                .map(|_| CacheAlignedCounter::new())
                .collect(),
            conflicts_per_shard: (0..shard_count)
                .map(|_| CacheAlignedCounter::new())
                .collect(),
...
impl DashMapOptimizer {
    ...
    /// 고성능 Lock-free 읽기
    pub fn read_with_operation<K, V, F, R>(
        &self,
        map: &DashMap<K, V>,
        key: &K,
        operation: F,
    ...
    /// 배치 읽기 최적화
    pub fn read_batch_with_operation<K, V, F, R>(
        &self,
        map: &DashMap<K, V>,
        keys: &[K],
        operation: F,
    ...
    pub fn collect_performance_stats(&self) -> DashMapPerformanceStats {
        ...
        // 로드팩터 계산
        let shard_loads: Vec<usize> = self
            .shard_stats
            .entries_per_shard
            .iter()
