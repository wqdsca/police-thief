...
impl ParallelBroadcaster {
    ...
    pub fn broadcast_parallel<T, F>(&self, message: &T, recipients: Vec<u32>, processor: F) -> Duration
    where
        T: Clone + Send + Sync,
    ...
    pub fn calculate_efficiency(&self) -> f64 {
        let total_utilization: f64 = self.worker_stats
            .iter()
    ...
    pub fn check_load_balance(&self) -> f64 {
        ...
        let utilizations: Vec<f64> = self.worker_stats
            .iter()
            .map(|stats| stats.utilization_rate())
            .collect();
...
impl ParallelUtils {
    ...
    pub fn optimize_batch_size(total_items: usize, thread_count: usize, item_size: usize) -> usize {
        ...
        // 캐시 라인의 배수로 조정
        (base_batch.div_ceil(cache_line_items) * cache_line_items)
            .clamp(1, 1000) // 1~1000개 제한
    }
...
mod tests {
    ...
    fn test_parallel_broadcaster() {
        ...
        let duration = broadcaster.broadcast_parallel(
            &message,
            recipients,
            |msg, recipient_id| {
                // 메시지 처리 시뮬레이션
