...
pub mod mock_compression {
    ...
    pub fn mock_lz4_compress(data: &[u8]) -> CompressionResult {
        ...
        CompressionResult::new(compressed, data.len(), compression_time, CompressionAlgorithm::LZ4)
    }
    ...
    pub fn mock_zstd_compress(data: &[u8]) -> CompressionResult {
        ...
        if result.compressed_size > 10 {
            result.compressed_data.truncate(result.compressed_size * 8 / 10);
        }
...
mod tests {
    ...
    fn test_mock_compression() {
        ...
        // 압축 해제 테스트
        let decompressed = mock_lz4_decompress(&result.compressed_data).unwrap();
