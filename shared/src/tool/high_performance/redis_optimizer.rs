...
impl RedisOptimizer {
    ...
    pub async fn get_multiple_keys(&self, keys: &[String]) -> Result<Vec<Option<Vec<u8>>>> {
        ...
        if !missing_keys.is_empty() {
            ...
            for (redis_idx, original_idx) in missing_indices.into_iter().enumerate() {
                if let Some(Some(value)) = redis_results.get(redis_idx) {
                    self.store_in_local_cache(&keys[original_idx], value.clone())
                        .await;
        ...
        stats.avg_response_time_ms = (stats.avg_response_time_ms
            * (stats.total_operations - keys.len() as u64) as f64
            + start_time.elapsed().as_millis() as f64)
            / stats.total_operations as f64;
    ...
    pub async fn execute_batch(&self, operations: Vec<BatchOperation>) -> Result<Vec<BatchResult>> {
        ...
        // 배치를 청크로 나누기
        let chunks: Vec<_> = operations.chunks(self.config.pipeline_batch_size).collect();
        ...
        stats.avg_response_time_ms = (stats.avg_response_time_ms
            * (stats.total_operations - operations.len() as u64) as f64
            + start_time.elapsed().as_millis() as f64)
            / stats.total_operations as f64;
    ...
    /// 파이프라인 청크 실행
    async fn execute_pipeline_chunk(
        &self,
        ...
        // 결과 변환
        for (i, (operation, redis_value)) in
            operations.iter().zip(pipeline_result.iter()).enumerate()
        {
    ...
    pub async fn get_stats(&self) -> RedisPerformanceStats {
        ...
        result.connection_pool_usage = (self.config.connection_pool_size - available_permits)
            as f64
            / self.config.connection_pool_size as f64
            * 100.0;
...
mod tests {
    ...
    async fn test_redis_optimizer() {
        ...
        if let Ok(optimizer) = RedisOptimizer::new(redis_url, RedisOptimizerConfig::default()).await
        {
            ...
            let results = optimizer.execute_batch(operations).await.ok();
