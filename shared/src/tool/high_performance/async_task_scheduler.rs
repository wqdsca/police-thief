...
impl AsyncTask {
    ...
    pub fn submitted_at(&self) -> Instant { self.submitted_at }
...
pub struct AsyncWorker {
    id: usize,
    task_queue: Arc<Mutex<BinaryHeap<Reverse<AsyncTask>>>>,
...
impl AsyncWorker {
    ...
    pub async fn submit_task(&self, task: AsyncTask) {
        {
            ...
            self.stats.current_load.store(queue.len(), Ordering::Relaxed);
        }
    ...
    pub async fn try_steal_task(&self, from_worker: &AsyncWorker) -> Option<AsyncTask> {
        ...
        let mut from_queue = from_worker.task_queue.lock().await;
        if from_queue.len() > 1 { // 최소 2개는 있어야 스틸링
            let stolen_task = from_queue.pop()?.0;
            from_worker.stats.current_load.store(from_queue.len(), Ordering::Relaxed);
            from_worker.stats.tasks_given.fetch_add(1, Ordering::Relaxed);
    ...
    pub async fn run(&self, other_workers: Arc<Vec<Arc<AsyncWorker>>>) {
        ...
        while !self.shutdown_signal.load(Ordering::Relaxed) {
            ...
            if let Some(task) = task {
                ...
                // 데드라인 체크
                if task.is_overdue() {
                    warn!("작업 {} 데드라인 초과 - 우선순위: {:?}", task.id(), task.priority());
                }
                
                // 작업 실행
                debug!("워커 {} 작업 {} 실행 시작 (우선순위: {:?})", 
                       self.id, task.id(), task.priority());
                ...
                debug!("워커 {} 작업 {} 완료 ({}μs)", 
                       self.id, task_id, duration.as_micros());
            } else {
                // 작업이 없으면 유휴 상태로 전환
...
impl AsyncTaskScheduler {
    ...
    pub async fn start(&self) {
        ...
        if self.config.enable_dynamic_scaling {
            let scheduler_clone = self.clone_for_balancing();
            tokio::spawn(async move {
                scheduler_clone.load_balancing_loop().await;
            });
        }
        
        info!("✅ 비동기 스케줄러 시작 완료 - {} 워커 활성화", self.config.worker_count);
    }
    ...
    async fn schedule_task(&self, task: AsyncTask) -> Result<(), &'static str> {
        self.scheduler_stats.total_tasks_submitted.fetch_add(1, Ordering::Relaxed);
        ...
        debug!("작업 {} 워커 {}에 할당 (우선순위: {:?})", 
               task.id(), worker.id, task.priority());
    ...
    async fn load_balancing_loop(&self) {
        let mut interval = tokio::time::interval(
            Duration::from_millis(self.config.load_balancing_interval_ms)
        );
        ...
        while !self.shutdown_signal.load(Ordering::Relaxed) {
            ...
            // 부하 불균형이 심한 경우 경고
            if max_load > min_load * 3 && total_load > 0 {
                warn!("워커 부하 불균형 감지: 최대 {} vs 최소 {}", max_load, min_load);
                self.scheduler_stats.load_balancing_events.fetch_add(1, Ordering::Relaxed);
            }
        }
    }
    ...
    pub async fn get_performance_report(&self) -> String {
        ...
        for worker in self.workers.iter() {
            ...
            worker_stats.push(format!(
                ...
                worker.id, processed, stolen, latency / 1000, queue_size
            ));
        ...
        format!(
            ...
            avg_latency / 1000,
            worker_stats.join("\n"),
            self.scheduler_stats.load_balancing_events.load(Ordering::Relaxed),
            self.scheduler_stats.total_tasks_submitted.load(Ordering::Relaxed)
        )
    }
    
    /// 로드 밸런싱용 클론
    fn clone_for_balancing(&self) -> BalancingScheduler {
        BalancingScheduler {
            workers: self.workers.clone(),
            shutdown_signal: self.shutdown_signal.clone(),
            config: self.config.clone(),
            stats: self.scheduler_stats.clone(),
        }
    }
}

/// 로드 밸런싱 전용 스케줄러 (lifetime 문제 해결)
struct BalancingScheduler {
    workers: Arc<Vec<Arc<AsyncWorker>>>,
    shutdown_signal: Arc<AtomicBool>,
    config: SchedulerConfig,
...
mod tests {
    ...
    async fn test_task_scheduler_basic() {
        ...
        // 간단한 작업 스케줄링
        let result = scheduler.schedule(async {
    ...
    async fn test_priority_scheduling() {
        ...
        // 여러 우선순위 작업 제출
        scheduler.schedule(async {}, TaskPriority::Low).await.unwrap();
        scheduler.schedule(async {}, TaskPriority::Critical).await.unwrap();
        scheduler.schedule(async {}, TaskPriority::High).await.unwrap();
