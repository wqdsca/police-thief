...
impl<T> LockFreeSPSCQueue<T> {
    pub fn new(capacity: usize) -> Self {
        ...
        for _ in 0..capacity {
            buffer.push(QueueNode { data: None });
        }
    ...
    pub fn enqueue(&self, item: T) -> bool {
        ...
        // SAFETY: tail index는 항상 버퍼 범위 내에 있음이 보장됨
        // capacity로 모듈로 연산을 수행하여 범위를 벗어날 수 없음
        unsafe {
            let node = &mut *(self.buffer.as_ptr().add(tail) as *mut QueueNode<T>);
            node.data = Some(item);
        }
    ...
    pub fn dequeue(&self) -> Option<T> {
        ...
        // SAFETY: head index는 항상 버퍼 범위 내에 있음이 보장됨
        // capacity로 모듈로 연산을 수행하여 범위를 벗어날 수 없음
        let item = unsafe {
            let node = &mut *(self.buffer.as_ptr().add(head) as *mut QueueNode<T>);
            node.data.take()
        };
