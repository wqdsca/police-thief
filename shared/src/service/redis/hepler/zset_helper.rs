...
impl ZSetHelper {
    ...
    pub async fn get_top_members(&self, id: u16, count: i64) -> Result<Vec<(String, f64)>> {
        ...
        RETRY_OPT
            .execute::<Vec<(String, f64)>, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.zrevrange_withscores(&key, 0, (count - 1).try_into().unwrap_or_default())
                        .await
    ...
    pub async fn get_bottom_members(&self, id: u16, count: i64) -> Result<Vec<(String, f64)>> {
        ...
        RETRY_OPT
            .execute::<Vec<(String, f64)>, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.zrange_withscores(&key, 0, (count - 1).try_into().unwrap_or_default())
                        .await
                        .context("ZSetHelper: ZRANGE WITHSCORES 실패")
                }
            })
    ...
    pub async fn range_with_scores<T: DeserializeOwned>(
        ...
        let rows: Vec<(String, f64)> = RETRY_OPT
            .execute::<Vec<(String, f64)>, _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = self.conn.get_connection();
                    if desc {
                        conn.zrevrange_withscores(
                            ...
                            stop.try_into().unwrap_or_default(),
                        )
                        .await
                        .context("ZSetHelper: ZREVRANGE WITHSCORES 실패")
                    } else {
                        conn.zrange_withscores(
                            &key,
                            start.try_into().unwrap_or(0),
                            stop.try_into().unwrap_or_default(),
                        )
        ...
        for (m, s) in rows {
            let v =
                serde_json::from_str::<T>(&m).context("ZSetHelper: JSON 역직렬화 실패(range)")?;
            out.push((v, s));
        }
        Ok(out)
    }
    ...
    /// 점수 범위 삭제
    pub async fn remove_by_score_range(
        &self,
        id: u16,
        min_score: f64,
        max_score: f64,
    ...
    pub async fn get_member_rank(&self, id: u16, member: &str) -> Result<Option<u64>> {
        let key = self.key.get_key(&id);
        let rank: Option<i64> = RETRY_OPT
            .execute::<Option<i64>, _, _>(|| {
                let key = key.clone();
                let member = member.to_string();
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.zrevrank(&key, member)
                        .await
                        .context("ZSetHelper: ZREVRANK 실패")
                }
            })
            .await?;
    ...
    /// 점수 범위 내 멤버 수
    pub async fn count_by_score_range(
        &self,
        id: u16,
        min_score: f64,
        max_score: f64,
    ...
    pub async fn get_range_by_score(&self, min_score: f64, max_score: f64) -> Result<Vec<String>> {
        ...
        println!(
            "DEBUG ZSetHelper: key={}, min_score={}, max_score={}, limit={}",
            key,
            min_score,
            max_score,
            self.limit.unwrap_or(20)
        ); // DEBUG 로그 추가
        println!("DEBUG ZSetHelper: min_score={min_score:?}, max_score={max_score:?}"); // 실제 값 확인

        let result = RETRY_OPT
            .execute::<Vec<String>, _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = ConnectionPool::get_connection()
                        .await
                    ...
                    println!(
                        "DEBUG: ZREVRANGEBYSCORE {} {} {} LIMIT 0 {}",
                        key,
                        max_arg,
                        min_arg,
                        self.limit.unwrap_or(20)
    ...
    pub async fn get_zset_stats(&self, id: u16) -> Result<(u64, f64, f64)> {
        ...
        let (member_count, min_range, max_range): (u64, ScoreRange, ScoreRange) = RETRY_OPT
            .execute::<(u64, Vec<(String, f64)>, Vec<(String, f64)>), _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = self.conn.get_connection();
