...
impl HashHelper {
    ...
    pub async fn set_hash_field<T: Serialize>(
        ...
        self.validator
            .validate_operation("HSET", &key, Some(field), Some(json.as_bytes()))
    ...
    pub async fn get_hash_field<T: DeserializeOwned>(
        ...
        self.validator
            .validate_operation("HGET", &key, Some(field), None)
    ...
    pub async fn get_all_hash_fields(
        ...
        RETRY_OPT
            .execute::<std::collections::HashMap<String, String>, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.hgetall(&key).await.context("HashHelper: HGETALL 실패")
                }
            })
            .await
    }
    ...
    pub async fn delete_hash_field(&self, id: u16, field: &str) -> Result<u64> {
        ...
        self.validator
            .validate_operation("HDEL", &key, Some(field), None)
    ...
    pub async fn is_exists_hash_id(&self, id: u16) -> Result<bool> {
        ...
        RETRY_OPT
            .execute::<bool, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.exists(&key).await.context("HashHelper: EXISTS 실패")
                }
            })
            .await
    }
    ...
    pub async fn mget_hash_fields(&self, id: u16, fields: &[&str]) -> Result<Vec<Option<String>>> {
        let key = self.key.get_key(&id);
    ...
    pub async fn mset_hash_fields(&self, id: u16, fields: &[(&str, &str)]) -> Result<()> {
        let key = self.key.get_key(&id);
