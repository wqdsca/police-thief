...
impl SetHelper {
    ...
    /// 멤버 조회 (JSON 역직렬화)
    pub async fn get_member<T: DeserializeOwned>(&self, id: u16, member: &str) -> Result<Option<T>> {
        let key = self.key.get_key(&id);

        let exists: bool = RETRY_OPT.execute::<bool, _, _>(|| {
            let key = key.clone();
            let _member = member.to_string();
            async move {
                let mut conn = self.conn.get_connection();
        ...
        if exists {
            // 멤버가 존재하면 raw 데이터 조회
            let raw: Option<String> = RETRY_OPT.execute::<Option<String>, _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = self.conn.get_connection();
            ...
            if let Some(json) = raw {
                let data = serde_json::from_str::<T>(&json)
                    .context("SetHelper: JSON 역직렬화 실패")?;
                Ok(Some(data))
            } else {
                Ok(None)
            }
    ...
    pub async fn get_member_raw(&self, id: u16, member: &str) -> Result<Option<String>> {
        ...
        if exists {
            RETRY_OPT.execute::<Option<String>, _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.srandmember(&key)
    ...
    pub async fn get_all_members<T: DeserializeOwned>(&self, id: u16) -> Result<Vec<T>> {
        ...
        let members: Vec<String> = RETRY_OPT.execute::<Vec<String>, _, _>(|| {
            let key = key.clone();
            async move {
                let mut conn = self.conn.get_connection();
                conn.smembers(&key)
    ...
    pub async fn get_all_members_raw(&self, id: u16) -> Result<Vec<String>> {
        ...
        RETRY_OPT.execute::<Vec<String>, _, _>(|| {
            let key = key.clone();
            async move {
                let mut conn = self.conn.get_connection();
                conn.smembers(&key)
    ...
    pub async fn get_random_member<T: DeserializeOwned>(&self, id: u16) -> Result<Option<T>> {
        ...
        let member: Option<String> = RETRY_OPT.execute::<Option<String>, _, _>(|| {
            let key = key.clone();
            async move {
                let mut conn = self.conn.get_connection();
                conn.srandmember(&key)
    ...
    /// 랜덤 멤버들 조회 (JSON 역직렬화)
    pub async fn get_random_members<T: DeserializeOwned>(&self, id: u16, count: i64) -> Result<Vec<T>> {
        let key = self.key.get_key(&id);
        ...
        let members: Vec<String> = RETRY_OPT.execute::<Vec<String>, _, _>(|| {
            ...
            async move {
                let mut conn = self.conn.get_connection();
                conn.srandmember_multiple(&key, count.try_into().unwrap())
                    .await
                    .context("SetHelper: SRANDMEMBER 실패")
            }
        }).await?;
    ...
    /// Set 연산: 교집합
    pub async fn intersection<T: DeserializeOwned>(&self, id: u16, other_keys: &[&str]) -> Result<Vec<T>> {
        let key = self.key.get_key(&id);
        let all_keys = std::iter::once(key.as_str()).chain(other_keys.iter().copied()).collect::<Vec<_>>();
    ...
    /// Set 연산: 차집합
    pub async fn difference<T: DeserializeOwned>(&self, id: u16, other_keys: &[&str]) -> Result<Vec<T>> {
        let key = self.key.get_key(&id);
        let all_keys = std::iter::once(key.as_str()).chain(other_keys.iter().copied()).collect::<Vec<_>>();
