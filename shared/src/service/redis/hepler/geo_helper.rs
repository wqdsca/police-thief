...
impl GeoHelper {
    ...
    pub async fn get_location<T: DeserializeOwned>(
        ...
        let result: Option<(String, f64, f64)> = RETRY_OPT.execute::<Option<(String, f64, f64)>, _, _>(|| {
            let key = key.clone();
        ...
        if let Some((json, longitude, latitude)) = result {
            let data = serde_json::from_str::<T>(&json)
                .context("GeoHelper: JSON 역직렬화 실패")?;
    ...
    pub async fn get_location_raw(
        ...
        RETRY_OPT.execute::<Option<(String, f64, f64)>, _, _>(|| {
            let key = key.clone();
            let member = member.to_string();
            async move {
                let mut conn = self.conn.get_connection();
    ...
    pub async fn get_locations_in_radius<T: DeserializeOwned>(
        ...
        let results: Vec<(String, f64, f64, f64)> = RETRY_OPT.execute::<Vec<(String, f64, f64, f64)>, _, _>(|| {
            let key = key.clone();
    ...
    pub async fn get_locations_in_radius_raw(
        ...
        RETRY_OPT.execute::<Vec<(String, f64, f64, f64)>, _, _>(|| {
            let key = key.clone();
            let unit = unit.to_string();
            async move {
                let mut conn = self.conn.get_connection();
    ...
    pub async fn get_geohash(&self, id: u16, member: &str) -> Result<Option<String>> {
        ...
        let hashes: Vec<Option<String>> = RETRY_OPT.execute::<Vec<Option<String>>, _, _>(|| {
            let key = key.clone();
            let member = member.to_string();
            async move {
                let mut conn = self.conn.get_connection();
    ...
    pub async fn get_geohashes(&self, id: u16, members: &[&str]) -> Result<Vec<Option<String>>> {
        ...
        RETRY_OPT.execute::<Vec<Option<String>>, _, _>(|| {
            let key = key.clone();
    ...
    pub async fn get_geo_stats(&self, id: u16) -> Result<(u64, f64, f64)> {
        ...
        Ok((location_count, min_score.unwrap_or(0.0), max_score.unwrap_or(0.0)))
    }
} 