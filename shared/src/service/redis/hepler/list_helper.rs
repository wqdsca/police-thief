...
impl ListHelper {
        ...
        let added = RETRY_OPT
            .execute::<u64, _, _>(|| {
                ...
                async move {
                    ...
                    let added: u64 =
                        FromRedisValue::from_redis_value(first).context("LPUSH 응답 파싱 실패")?;
                    Ok(added)
                }
            })
            .await?;
            ...
            .execute::<u64, _, _>(|| {
                ...
                async move {
                    ...
                    let first = resp
                        ...
                        .ok_or_else(|| anyhow!("파이프라인 응답 비어있음"))?;
                    let added: u64 =
                        FromRedisValue::from_redis_value(first).context("RPUSH 응답 파싱 실패")?;
                    Ok(added)
                }
            })
            .await?;
    ...
    pub async fn pop_front<T: DeserializeOwned>(&self, id: u16) -> Result<Option<T>> {
        ...
        let result: Option<String> = RETRY_OPT
            .execute::<Option<String>, _, _>(|| {
                let key = key.clone();
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.lpop(&key, None).await.context("ListHelper: LPOP 실패")
                }
            })
            .await?;
        ...
        if let Some(json) = result {
            let data =
                serde_json::from_str::<T>(&json).context("ListHelper: JSON 역직렬화 실패")?;
            Ok(Some(data))
        } else {
            Ok(None)
        }
    ...
    pub async fn pop_back<T: DeserializeOwned>(&self, id: u16) -> Result<Option<T>> {
        ...
        let result: Option<String> = RETRY_OPT
            .execute::<Option<String>, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.rpop(&key, None).await.context("ListHelper: RPOP 실패")
                }
            })
            .await?;
    ...
    pub async fn get_all<T: DeserializeOwned>(&self, id: u16) -> Result<Vec<T>> {
        ...
        let items: Vec<String> = RETRY_OPT
    ...
    pub async fn get_length(&self, id: u16) -> Result<u64> {
        ...
        RETRY_OPT
            .execute::<u64, _, _>(|| {
                ...
                async move {
                    let mut conn = self.conn.get_connection();
                    conn.llen(&key).await.context("ListHelper: LLEN 실패")
                }
            })
            .await
    }
