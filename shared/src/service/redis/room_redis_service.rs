use crate::config::{connection_pool::ConnectionPool, redis_config::RedisConfig};
...
impl RoomRedisService {
    ...
    pub async fn make_room(&self, room_info: RoomInfo) -> Result<bool, AppError> {
        ...
        p.hset_multiple(
            self.config.key_type.get_key(&room_id),
            &[
                ("room_name", &room_info.room_name),
                ("max_player_num", &room_info.max_player_num.to_string()),
                (
                    "current_player_num",
                    &room_info.current_player_num.to_string(),
                ),
    ...
    pub async fn get_room_list(&self, last_id: u16) -> Result<Vec<RoomInfo>, AppError> {
        ...
        let last_id_time = if last_id == 0 {
            ...
            zset_helper
                .get_member_score(last_id as i64)
                .await
                .map_err(|e| AppError::RedisConnection(e.to_string()))?
                .unwrap_or(0.0)
        };
        ...
        println!("DEBUG RoomRedisService: last_id={last_id}, last_id_time={last_id_time}");
        println!(
            "DEBUG RoomRedisService: NEG_INFINITY={}, last_id_time={}",
            f64::NEG_INFINITY,
            last_id_time
        );
        ...
        println!(
            "DEBUG RoomRedisService: room_id_list.len()={}",
            room_id_list.len()
        );
