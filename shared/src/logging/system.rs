...
use crate::logging::{
    ...
    formatter::{LogEntry, LogFormatter, LogLevel},
...
impl LoggingSystem {
    ...
    pub async fn new<P: AsRef<Path>>(base_dir: P) -> Result<Self> {
        ...
        let rotation_manager = Arc::new(Mutex::new(LogRotationManager::new(
            &base_dir,
            config.clone(),
        )));
    ...
    async fn create_log_writer(&self, service_type: ServiceType) -> Result<()> {
        ...
        let writer =
            AsyncLogWriter::new(log_file_path, self.config.clone(), self.formatter.clone()).await?;

        let mut writers = self.writers.write().await;
        writers.insert(service_type, Arc::new(writer));

        debug!(service = service_type.as_str(), "로그 작성기 생성됨");
            ...
            let rotation_task = tokio::spawn(async move {
                ...
                loop {
                    ...
                    if let Ok(new_writer) =
                        AsyncLogWriter::new(new_log_path, config.clone(), formatter.clone()).await
                    {
    ...
    pub async fn log<S: AsRef<str>>(&self, level: LogLevel, message: S, context: &[(&str, &str)]) {
        ...
        let entry = LogEntry::new(level, service_name, message.as_ref().to_string(), context);
...
/// 전역 로깅 시스템 인스턴스 (싱글톤)
static GLOBAL_LOGGER: tokio::sync::OnceCell<Arc<Mutex<LoggingSystem>>> =
    tokio::sync::OnceCell::const_new();

/// 전역 로깅 시스템 초기화
pub async fn init_global_logging(service_type: ServiceType, base_dir: Option<&Path>) -> Result<()> {
    ...
    GLOBAL_LOGGER
        .set(Arc::new(Mutex::new(logging_system)))
...
mod tests {
    ...
    async fn test_logging_system_creation() {
        ...
        let system = LoggingSystem::new(temp_dir.path())
            .await
    ...
    async fn test_logging_system_initialization() {
        let temp_dir = TempDir::new().expect("Test assertion failed");
        let mut system = LoggingSystem::new(temp_dir.path())
            .await
            .expect("Test assertion failed");

        system
            .init(ServiceType::GrpcServer)
            .await
            .expect("Test assertion failed");
    ...
    async fn test_test_mode_logging() {
        let mut system = LoggingSystem::new_test_mode()
            .await
            .expect("Test assertion failed");
        system
            .init(ServiceType::GrpcServer)
    ...
    async fn test_log_levels() {
        ...
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Trace\"") && log.contains("Trace message")));
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Debug\"") && log.contains("Debug message")));
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Info\"") && log.contains("Info message")));
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Warn\"") && log.contains("Warn message")));
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Error\"") && log.contains("Error message")));
        assert!(logs
            .iter()
            .any(|log| log.contains("\"level\":\"Fatal\"") && log.contains("Fatal message")));
    }
    ...
    async fn test_global_logging() {
        init_global_logging(ServiceType::GameCenter, None)
            .await
            .expect("Test assertion failed");

        log_info("Global test message", &[("test", "global")]).await;

        let logger = get_global_logger().await.expect("Test assertion failed");
        let logger = logger.lock().await;
