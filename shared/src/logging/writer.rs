...
use crate::logging::formatter::{LogEntry, LogFormatter};
...
impl AsyncLogWriter {
    ...
    async fn flush_buffer(
        ...
        writer.flush().await.context("로그 파일 플러시 실패")?;
...
mod tests {
    use super::*;
    use crate::logging::formatter::{LogEntry, LogFormatter, LogLevel};
    use std::sync::Arc;
    ...
    async fn test_async_log_writer() {
        ...
        let writer = AsyncLogWriter::new(log_path.clone(), config, formatter)
            .await
        ...
        writer.write_log(entry).expect("Test assertion failed");
        ...
        // 정리
        writer.shutdown().await.expect("Test assertion failed");
    }
