...
impl LogRotationManager {
    ...
    pub async fn get_current_log_file(&mut self, service_type: ServiceType) -> Result<PathBuf> {
        ...
        self.current_files.insert(service_type, expected_path.clone());
    ...
    pub async fn cleanup_old_logs(&mut self) -> Result<usize> {
        ...
        for service_type in [
            ...
            deleted_count += self.cleanup_service_logs(&service_dir, &cutoff_date).await?;
        }
    ...
    async fn cleanup_service_logs(&self, service_dir: &Path, cutoff_date: &DateTime<Utc>) -> Result<usize> {
        let mut deleted_count = 0;
...
mod tests {
    ...
    async fn test_create_log_file() {
        ...
        let log_path = manager.get_current_log_file(ServiceType::GrpcServer).await.unwrap();
    ...
    async fn test_file_rotation() {
        ...
        file.write_all(&vec![b'x'; 2048]).await.unwrap(); // 2KB
        file.flush().await.unwrap();
        drop(file);
        
        // 순환 실행
        let rotated = manager.rotate_if_needed(ServiceType::GrpcServer).await.unwrap();
        assert!(rotated.is_some());
        
        // 새 로그 파일이 생성되었는지 확인
        assert!(log_path.exists());
        let new_size = manager.check_file_size(&log_path).await.unwrap();
    ...
    async fn test_cleanup_old_logs() {
        ...
        // 현재 로그 파일 생성
        manager.get_current_log_file(ServiceType::GrpcServer).await.unwrap();
        
        // 오래된 로그 파일 생성 (수동으로 생성 시간 조작)
        let old_log_path = temp_dir.path()
            .join("grpcserver")
            .join("grpc_2023-01-01.log");
        File::create(&old_log_path).await.unwrap();
        
        // 정리 실행
        let deleted_count = manager.cleanup_old_logs().await.unwrap();
