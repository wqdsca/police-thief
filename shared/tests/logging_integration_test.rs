...
async fn test_logging_system_initialization() -> Result<()> {
    ...
    // 상태 확인
    assert_eq!(
        logger.get_state().await,
        shared::logging::system::LoggingState::Initialized
    );
...
async fn test_daily_log_file_creation() -> Result<()> {
    ...
    assert!(log_file_path.extension().expect("Test assertion failed") == "log");
...
async fn test_log_levels_writing() -> Result<()> {
    ...
    // 각 레벨별 로그 작성
    system
        .trace("Trace level message", &[("trace_data", "trace_value")])
        .await;
    system
        .debug("Debug level message", &[("debug_data", "debug_value")])
        .await;
    system
        .info("Info level message", &[("info_data", "info_value")])
        .await;
    system
        .warn("Warn level message", &[("warn_data", "warn_value")])
        .await;
    system
        .error("Error level message", &[("error_data", "error_value")])
        .await;
    system
        .fatal("Fatal level message", &[("fatal_data", "fatal_value")])
        .await;
    ...
    for (i, log) in logs.iter().enumerate() {
        assert!(
            log.contains(levels[i]),
            "로그에 레벨이 포함되지 않음: {}",
            levels[i]
        );
        assert!(
            log.contains(messages[i]),
            "로그에 메시지가 포함되지 않음: {}",
            messages[i]
        );
...
async fn test_log_rotation() -> Result<()> {
    ...
    // 순환된 파일이 존재하는지 확인
    let rotated_path = rotated_file.expect("Test assertion failed");
...
async fn test_memory_writer() -> Result<()> {
    ...
    for (level_str, message, context) in entries {
        let level = level_str.parse().expect("Test assertion failed");
            ...
            for j in 0..10 {
                system
                    .info(
                        ...
                        &[("task_id", &i.to_string()), ("message_id", &j.to_string())],
                    )
                    .await;
            }
        });
...
async fn test_system_shutdown() -> Result<()> {
    ...
    // 로그 작성
    logger
        .info("Before shutdown", &[("status", "running")])
        .await;
...
async fn test_realistic_usage_scenario() -> Result<()> {
    ...
    logger
        .debug(
            "사용자 인증 시도",
            &[("user_id", "test_user"), ("ip", "127.0.0.1")],
        )
        .await;
