...
use room::{room_service_client::RoomServiceClient, GetRoomListRequest, MakeRoomRequest, RoomInfo};
use user::{user_service_client::UserServiceClient, LoginRequest};
...
async fn test_room_pagination(channel: Channel) -> Result<()> {
    ...
    match client.get_room_list(tonic::Request::new(request)).await {
        Ok(response) => {
            ...
            if !rooms.is_empty() {
                let last_id = rooms.last().expect("Test assertion failed").room_id;
                println!("\n   📄 두 번째 페이지 조회 (last_room_id = {})", last_id);

                let request2 = GetRoomListRequest {
                    last_room_id: last_id,
                };
                match client.get_room_list(tonic::Request::new(request2)).await {
                    Ok(response2) => {
                        ...
                        if !rooms2.is_empty() {
                            let last_id2 = rooms2.last().expect("Test assertion failed").room_id;
                            println!("\n   📄 세 번째 페이지 조회 (last_room_id = {})", last_id2);

                            let request3 = GetRoomListRequest {
                                last_room_id: last_id2,
                            };
...
fn validate_pagination_logic(
    ...
    let second_ids: std::collections::HashSet<i32> =
        second_page.iter().map(|r| r.room_id).collect();
    ...
    let invalid_ids: Vec<_> = second_page
        .iter()
...
async fn verify_redis_data() -> Result<()> {
        ...
        .find(|line| line.starts_with("used_memory_human:"))
    {
        println!(
            "   💾 Redis 메모리 사용량: {}",
            line.split(':').nth(1).unwrap_or("N/A")
        );
    }
