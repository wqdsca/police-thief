...
impl GameCenterServer {
    ...
    async fn start_redis_server(&mut self) -> Result<()> {
        ...
        let redis_check = Command::new("redis-cli").arg("ping").output().await;
        ...
        while retry_count < 5 {
            let ping_result = Command::new("redis-cli").arg("ping").output().await;
    ...
    async fn stop_redis_server(&mut self) -> Result<()> {
        ...
        // Redis 서버 종료 명령 전송
        let _ = Command::new("redis-cli").arg("SHUTDOWN").output().await;
    ...
    pub async fn start(&mut self) -> Result<()> {
        ...
        // Redis 연결 설정
        let redis_config = RedisConfig::new().await.context("RedisConfig 생성 실패")?;
        ...
        // 통합 서버 생성 및 시작
        let unified_server = UnifiedGameServer::from_env().context("통합 서버 설정 생성 실패")?;
...
async fn main() -> Result<()> {
    ...
    let result = match command {
        ...
        "status" => {
            ...
            let redis_status = Command::new("redis-cli").arg("ping").output().await;
        ...
        _ => {
            ...
            println!("사용 가능한 명령어: start, stop, test, server, grpc, tcp, status, help");
